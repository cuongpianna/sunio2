<?php
/**
 * Plugin Name:			sunio Sticky Header
 ...
 * Tested up to:		4.9.6
 *
 * Text Domain: sunio-sticky-header
 * Domain Path: /languages/
 *
 * @package sunio_Sticky_Header
 * @category Core
 * @author Aztheme
 */
...
/**
 * Returns the main instance of sunio_Sticky_Header to prevent the need to use globals.
 *
 * @since  1.0.0
 * @return object sunio_Sticky_Header
 */
function sunio_Sticky_Header() {
	return sunio_Sticky_Header::instance();
} // End sunio_Sticky_Header()

sunio_Sticky_Header();

/**
 * Main sunio_Sticky_Header Class
 *
 * @class sunio_Sticky_Header
 * @version	1.0.0
 * @since 1.0.0
 * @package	sunio_Sticky_Header
 */
final class sunio_Sticky_Header {
	/**
	 * sunio_Sticky_Header The single instance of sunio_Sticky_Header.
	 * @var 	object
	 * @access  private
	 * @since 	1.0.0
	 */
	...
	public function __construct() {
		$this->token 			= 'sunio-sticky-header';
	...
	public function updater() {
		...
		// Plugin Updater Code
		if( class_exists( 'Aztheme_Plugin_Updater' ) ) {
			$license	= new Aztheme_Plugin_Updater( __FILE__, 'Sticky Header', $this->version, 'sunio' );
		}
	}

	/**
	 * Main sunio_Sticky_Header Instance
	 *
	 * Ensures only one instance of sunio_Sticky_Header is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @see sunio_Sticky_Header()
	 * @return Main sunio_Sticky_Header instance
	 */
	...
	public function load_plugin_textdomain() {
		load_plugin_textdomain( 'sunio-sticky-header', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	}
	 ...
	 * Only executes if Aztheme or a child theme using Aztheme as a parent is active and the extension specific filter returns true.
	 * @return void
	 */
	public function setup() {
		
		// Capabilities
		$capabilities = apply_filters( 'aztheme_main_metaboxes_capabilities', 'manage_options' );
		...
		add_filter( 'sunio_header_classes', array( $this, 'header_classes' ) );
		add_filter( 'sunio_header_logo_classes', array( $this, 'logo_classes' ) );
	...
	public function customize_register( $wp_customize ) {
        ...
        $wp_customize->add_section( 'osh_section' , array(
		    'title'      	=> esc_html__( 'Sticky Header', 'sunio-sticky-header' ),
		    'priority'   	=> 210,
		) );
		...
		$wp_customize->add_control( new sunio_Customizer_Buttonset_Control( $wp_customize, 'osh_sticky_choose', array(
			'label'	   				=> esc_html__( 'Sticky', 'sunio-sticky-header' ),
		    'description'   		=> sprintf( esc_html__( 'This option has been designed for the Custom Header style. %1$sLearn more%2$s.', 'sunio-sticky-header' ), '<a href="http://docs.sunio.org/article/460-sticky-header-for-the-custom-header-style" target="_blank">', '</a>' ),
			'section'  				=> 'osh_section',
			'settings' 				=> 'osh_sticky_choose',
			'priority' 				=> 10,
			'choices' 				=> array(
				'auto' 		=> esc_html__( 'Auto', 'sunio-sticky-header' ),
				'manual' 	=> esc_html__( 'Manual', 'sunio-sticky-header' ),
			),
		) ) );
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'osh_has_sticky_topbar', array(
			'label'			=> esc_html__( 'Sticky Top Bar', 'sunio-sticky-header' ),
			'section'		=> 'osh_section',
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'osh_has_sticky_mobile', array(
			'label'			=> esc_html__( 'Sticky Mobile', 'sunio-sticky-header' ),
			'section'		=> 'osh_section',
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'osh_has_full_width_header', array(
			'label'			=> esc_html__( 'Full Width Scrolling', 'sunio-sticky-header' ),
			'section'		=> 'osh_section',
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'osh_no_shadow', array(
			'label'			=> esc_html__( 'No Shadow When Scrolling', 'sunio-sticky-header' ),
			'section'		=> 'osh_section',
		...
		$wp_customize->add_control( new sunio_Customizer_Buttonset_Control( $wp_customize, 'osh_sticky_header_style', array(
			'label'			=> esc_html__( 'Sticky Style', 'sunio-sticky-header' ),
			'section'		=> 'osh_section',
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'osh_sticky_header_effect', array(
			'label'	   				=> esc_html__( 'Sticky Effect', 'sunio-sticky-header' ),
			'description'	   		=> esc_html__( 'Do not work with all header styles.', 'sunio-sticky-header' ),
			'type' 					=> 'select',
			'section'  				=> 'osh_section',
			'settings' 				=> 'osh_sticky_header_effect',
			'priority' 				=> 10,
			'choices' 				=> array(
				'none' 		=> esc_html__( 'No Effect', 'sunio-sticky-header' ),
				'slide' 	=> esc_html__( 'Slide', 'sunio-sticky-header' ),
				'up'		=> esc_html__( 'Show Up/Hide Down', 'sunio-sticky-header' ),
			),
		) ) );
		...
		$wp_customize->add_control( new sunio_Customizer_Range_Control( $wp_customize, 'osh_shrink_header_height', array(
			'label'			=> esc_html__( 'Sticky Height', 'sunio-sticky-header' ),
			'section'		=> 'osh_section',
		...
		$wp_customize->add_control( new sunio_Customizer_Dimensions_Control( $wp_customize, 'osh_header_top_padding', array(
			'label'	   				=> esc_html__( 'Sticky Padding (px)', 'sunio-sticky-header' ),
			'section'  				=> 'osh_section',
			'settings' => array(
		...
		$wp_customize->add_control( new sunio_Customizer_Range_Control( $wp_customize, 'osh_sticky_header_opacity', array(
			'label'			=> esc_html__( 'Sticky Opacity', 'sunio-sticky-header' ),
			'section'		=> 'osh_section',
		...
		$wp_customize->add_control( new sunio_Customizer_Heading_Control( $wp_customize, 'osh_logo_heading', array(
			'label'    		=> esc_html__( 'Logo', 'sunio-sticky-header' ),
			'section'  		=> 'osh_section',
			'priority' 		=> 10,
		) ) );
		...
		$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'osh_logo', array(
			'label'	   				=> esc_html__( 'Sticky Logo', 'sunio-sticky-header' ),
			'description'	   		=> esc_html__( 'If you want to display a different logo when scrolling (optional)', 'sunio-sticky-header' ),
			'section'  				=> 'osh_section',
			'settings' 				=> 'osh_logo',
			'priority' 				=> 10,
		) ) );
		...
		$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'osh_logo_retina', array(
			'label'	   				=> esc_html__( 'Sticky Retina Logo', 'sunio-sticky-header' ),
			'section'  				=> 'osh_section',
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'osh_shrink_header_logo_height', array(
			'label'			=> esc_html__( 'Logo Height On Sticky', 'sunio-sticky-header' ),
			'section'		=> 'osh_section',
			'type'			=> 'number',
			'active_callback' => 'osh_cac_has_shrink_style',
		...
		$wp_customize->add_control( new sunio_Customizer_Heading_Control( $wp_customize, 'osh_styling_heading', array(
			'label'    		=> esc_html__( 'Styling', 'sunio-sticky-header' ),
			'section'  		=> 'osh_section',
			'priority' 		=> 10,
		) ) );
		...
		$wp_customize->add_control( new sunio_Customizer_Color_Control( $wp_customize, 'osh_background_color', array(
			'label'	   				=> esc_html__( 'Background Color', 'sunio-sticky-header' ),
			'section'  				=> 'osh_section',
		...
		$wp_customize->add_control( new sunio_Customizer_Color_Control( $wp_customize, 'osh_links_color', array(
			'label'	   				=> esc_html__( 'Links Color', 'sunio-sticky-header' ),
			'section'  				=> 'osh_section',
		...
		$wp_customize->add_control( new sunio_Customizer_Color_Control( $wp_customize, 'osh_links_hover_color', array(
			'label'	   				=> esc_html__( 'Links Color: Hover', 'sunio-sticky-header' ),
			'section'  				=> 'osh_section',
		...
		$wp_customize->add_control( new sunio_Customizer_Color_Control( $wp_customize, 'osh_links_active_color', array(
			'label'	   				=> esc_html__( 'Current Menu Item Color', 'sunio-sticky-header' ),
			'section'  				=> 'osh_section',
		...
		$wp_customize->add_control( new sunio_Customizer_Color_Control( $wp_customize, 'osh_links_bg_color', array(
			'label'	   				=> esc_html__( 'Links Background Color', 'sunio-sticky-header' ),
			'section'  				=> 'osh_section',
		...
		$wp_customize->add_control( new sunio_Customizer_Color_Control( $wp_customize, 'osh_links_hover_bg_color', array(
			'label'	   				=> esc_html__( 'Links Background Color: Hover', 'sunio-sticky-header' ),
			'section'  				=> 'osh_section',
		...
		$wp_customize->add_control( new sunio_Customizer_Color_Control( $wp_customize, 'osh_links_active_bg_color', array(
			'label'	   				=> esc_html__( 'Current Menu Item Background', 'sunio-sticky-header' ),
			'section'  				=> 'osh_section',
		...
		// $wp_customize->add_control( new Aztheme_Customizer_Color_Control( $wp_customize, 'osh_menu_social_links_color', array(
		// 	'label'	   				=> esc_html__( 'Simple Social Links Color', 'sunio' ),
		// 	'section'  				=> 'osh_section',
		...
		// $wp_customize->add_control( new Aztheme_Customizer_Color_Control( $wp_customize, 'osh_menu_social_hover_links_color', array(
		// 	'label'	   				=> esc_html__( 'Simple Social Links Color: Hover', 'sunio' ),
		// 	'section'  				=> 'osh_section',
	...
	public function new_tab( $butterbean, $post_type ) {
		...
		$manager->register_section(
	        'sunio_mb_sticky',
	        array(
	            'label' => esc_html__( 'Sticky Header', 'sunio-sticky-header' ),
		...
		$manager->register_control(
	        ...
	        array(
	            'section' 		=> 'sunio_mb_sticky',
	            'type'    		=> 'buttonset',
	            'label'   		=> esc_html__( 'Sticky Top Bar', 'sunio-sticky-header' ),
	            'description'   => esc_html__( 'Disable the sticky top bar on this page/post.', 'sunio-sticky-header' ),
				'choices' 		=> array(
					'default' 	=> esc_html__( 'Default', 'sunio-sticky-header' ),
					'off' 		=> esc_html__( 'Disable', 'sunio-sticky-header' ),
				),
	        )
	    );
		...
		$manager->register_control(
	        ...
	        array(
	            ...
	            'type'    		=> 'buttonset',
	            'label'   		=> esc_html__( 'Sticky Header', 'sunio-sticky-header' ),
	            'description'   => esc_html__( 'Disable the sticky header on this page/post.', 'sunio-sticky-header' ),
				'choices' 		=> array(
					'default' 	=> esc_html__( 'Default', 'sunio-sticky-header' ),
					'off' 		=> esc_html__( 'Disable', 'sunio-sticky-header' ),
				),
		...
		$manager->register_control(
	        ...
	        array(
	            ...
	            'type'    		=> 'buttonset',
	            'label'   		=> esc_html__( 'Sticky Style', 'sunio-sticky-header' ),
	            'description'   => esc_html__( 'Choose your sticky style on this page/post.', 'sunio-sticky-header' ),
				'choices' 		=> array(
					'default' 	=> esc_html__( 'Default', 'sunio-sticky-header' ),
					'shrink' 	=> esc_html__( 'Shrink', 'sunio-sticky-header' ),
					'fixed' 	=> esc_html__( 'Fixed', 'sunio-sticky-header' ),
				),
	        )
	    );
	    ...
	    $manager->register_control(
	        ...
	        array(
	            'section' 		=> 'sunio_mb_sticky',
	            'type'    		=> 'select',
	            'label'   		=> esc_html__( 'Sticky Effect', 'sunio-extra' ),
	            'description'   => esc_html__( 'Select your sticky effect. Do not work with all header styles.', 'sunio-extra' ),
				'choices' 		=> array(
					'' 		=> esc_html__( 'No Effect', 'sunio-extra' ),
					'slide' => esc_html__( 'Slide', 'sunio-extra' ),
					'up' 	=> esc_html__( 'Show Up/Hide Down', 'sunio-extra' ),
				),
	        )
	    );
		...
		$manager->register_control(
	        ...
	        array(
	            ...
	            'type'    		=> 'image',
	            'label'   		=> esc_html__( 'Logo', 'sunio-sticky-header' ),
	            'description'   => esc_html__( 'Select a custom sticky logo on this page/post.', 'sunio-sticky-header' ),
	        )
	    );
		...
		$manager->register_control(
	        ...
	        array(
	            'section' 		=> 'sunio_mb_sticky',
	            'type'    		=> 'image',
	            'label'   		=> esc_html__( 'Retina Logo', 'sunio-sticky-header' ),
	            'description'   => esc_html__( 'Select a custom retina sticky logo on this page/post.', 'sunio-sticky-header' ),
	        )
	    );
	    ...
	    $manager->register_control(
	        ...
	        array(
	            'section' 		=> 'sunio_mb_sticky',
	            'type'    		=> 'number',
	            'label'   		=> esc_html__( 'Logo Height On Sticky (px)', 'sunio-sticky-header' ),
	            'description'   => esc_html__( 'Enter the height of your logo when you scroll.', 'sunio-sticky-header' ),
	            'attr'    		=> array(
					'min' 	=> '10',
					'step' 	=> '1',
				),
		...
		$manager->register_control(
	        ...
	        array(
	            ...
	            'type'    		=> 'rgba-color',
	            'label'   		=> esc_html__( 'Background Color', 'sunio-sticky-header' ),
	            'description'   => esc_html__( 'Select a color. Hex code, ex: #555', 'sunio-sticky-header' ),
	        )
	    );
	    ...
	    $manager->register_control(
	        ...
	        array(
	            ...
	            'type'    		=> 'rgba-color',
	            'label'   		=> esc_html__( 'Links Color', 'sunio-sticky-header' ),
	            'description'   => esc_html__( 'Select a color. Hex code, ex: #fff', 'sunio-sticky-header' ),
	        )
	    );
		...
		$manager->register_control(
	        ...
	        array(
	            ...
	            'type'    		=> 'rgba-color',
	            'label'   		=> esc_html__( 'Links Color: Hover', 'sunio-sticky-header' ),
	            'description'   => esc_html__( 'Select a color. Hex code, ex: #13aff0', 'sunio-sticky-header' ),
	        )
	    );
		...
		$manager->register_control(
	        ...
	        array(
	            ...
	            'type'    		=> 'rgba-color',
	            'label'   		=> esc_html__( 'Current Menu Item Color', 'sunio-sticky-header' ),
	            'description'   => esc_html__( 'Select a color. Hex code, ex: #333', 'sunio-sticky-header' ),
	        )
	    );
		...
		$manager->register_control(
	        ...
	        array(
	            'section' 		=> 'sunio_mb_sticky',
	            'type'    		=> 'rgba-color',
	            'label'   		=> esc_html__( 'Simple Social Links Color', 'sunio-sticky-header' ),
	            'description'   => esc_html__( 'Select a color. Hex code, ex: #fff', 'sunio-sticky-header' ),
	        )
	    );
		...
		$manager->register_control(
	        ...
	        array(
	            'section' 		=> 'sunio_mb_sticky',
	            'type'    		=> 'rgba-color',
	            'label'   		=> esc_html__( 'Simple Social Links Color: Hover', 'sunio-sticky-header' ),
	            'description'   => esc_html__( 'Select a color. Hex code, ex: #13aff0', 'sunio-sticky-header' ),
	        )
	    );
	...
	public function custom_sticky_style( $style ) {
		...
		$meta = get_post_meta( sunio_post_id(), 'osh_sticky_header_style', true );
	...
	public function custom_sticky_effect( $sffect ) {
		...
		$meta = get_post_meta( sunio_post_id(), 'osh_sticky_header_effect', true );
	...
	public function sticky_logo( $logo_url ) {
		...
		if ( $meta = get_post_meta( sunio_post_id(), 'osh_custom_sticky_logo', true ) ) {
			$logo_url = $meta;
	...
	public function retina_sticky_logo( $logo_url ) {
		...
		if ( $meta = get_post_meta( sunio_post_id(), 'osh_custom_retina_sticky_logo', true ) ) {
			$logo_url = $meta;
	...
	public function sticky_logo_height( $logo_height ) {
		...
		if ( $meta = get_post_meta( sunio_post_id(), 'osh_custom_sticky_logo_height', true ) ) {
			$logo_height = $meta;
		}
	...
	public function background_color( $color ) {
		...
		if ( $meta = get_post_meta( sunio_post_id(), 'osh_background_color', true ) ) {
			$color = $meta;
		}
	...
	public function links_color( $color ) {
		...
		if ( $meta = get_post_meta( sunio_post_id(), 'osh_links_color', true ) ) {
			$color = $meta;
		}
	...
	public function links_hover_color( $color ) {
		...
		if ( $meta = get_post_meta( sunio_post_id(), 'osh_links_hover_color', true ) ) {
			$color = $meta;
		}
	...
	public function links_active_color( $color ) {
		...
		if ( $meta = get_post_meta( sunio_post_id(), 'osh_links_active_color', true ) ) {
			$color = $meta;
		}
	...
	public function links_bg_color( $color ) {
		...
		if ( $meta = get_post_meta( sunio_post_id(), 'osh_links_bg_color', true ) ) {
			$color = $meta;
		}
	...
	public function links_hover_bg_color( $color ) {
		...
		if ( $meta = get_post_meta( sunio_post_id(), 'osh_links_hover_bg_color', true ) ) {
			$color = $meta;
		}
	...
	public function links_active_bg_color( $color ) {
		...
		if ( $meta = get_post_meta( sunio_post_id(), 'osh_links_active_bg_color', true ) ) {
			$color = $meta;
		}
	...
	public function menu_social_links_color( $color ) {
		...
		if ( $meta = get_post_meta( sunio_post_id(), 'osh_menu_social_links_color', true ) ) {
			$color = $meta;
		}
	...
	public function menu_social_hover_links_color( $color ) {
		...
		if ( $meta = get_post_meta( sunio_post_id(), 'osh_menu_social_hover_links_color', true ) ) {
			$color = $meta;
		}
	...
	public function scripts() {
		...
		wp_enqueue_script( 'osh-js-scripts', plugins_url( '/assets/js/main.js', __FILE__ ), array( 'jquery' ), null, true );
	...
	public function if_topbar_sticky() {
		...
		$meta = sunio_post_id() ? get_post_meta( sunio_post_id(), 'osh_disable_topbar_sticky', true ) : '';
	...
	public function topbar_sticky() {
		if ( true == $this->if_topbar_sticky() ) {
			add_filter( 'sunio_topbar_classes', array( $this, 'topbar_classes' ) );
		}
	}

	/**
	 * Aztheme Sticky Top Bar Class
	...
	public function if_header_sticky() {
		...
		// Retunr meta if Disable if selected
		$meta = sunio_post_id() ? get_post_meta( sunio_post_id(), 'osh_disable_header_sticky', true ) : '';
	...
	public function header_classes( $classes ) {
		...
		// Get header style
		$headerStyle = sunio_header_style();
	...
	public function head_css( $output ) {
		...
		if ( 'top' != $headerStyle && 'fixed' != $this->sticky_style() ) {
			...
			if ( ! empty( $shrink_height ) && '54' != $shrink_height ) {
				$css .= '.is-sticky #site-header.shrink-header #site-logo{line-height:'. $shrink_height .'px;}';
				$css .= '.is-sticky #site-header.shrink-header #site-logo #site-logo-inner, .is-sticky #site-header.shrink-header .sunio-social-menu .social-menu-inner, .is-sticky #site-header.shrink-header.full_screen-header .menu-bar-inner,.after-header-content .after-header-content-inner{height:'. $shrink_height .'px;}';
				$css .= '.is-sticky #site-header.shrink-header #site-logo #site-logo-inner,.is-sticky #site-header.shrink-header #site-navigation-wrap .dropdown-menu > li > a, .is-sticky #site-header.shrink-header .sunio-mobile-menu-icon a,.after-header-content .after-header-content-inner > a,.after-header-content .after-header-content-inner > div > a{line-height:'. $shrink_height .'px;}';
				$css .= '.is-sticky #site-header.shrink-header .header_seach .header-searchform-wrap .header-searchform input{min-height:'. $shrink_height .'px;}';
			}
		...
		if ( isset( $top_padding ) && '8' != $top_padding && '' != $top_padding
			...
			$css .= 'body .is-sticky #site-header.fixed-scroll #site-header-inner{padding:'. sunio_spacing_css( $top_padding, $right_padding, $bottom_padding, $left_padding ) .'}';
		}
		...
		if ( isset( $tablet_top_padding ) && '' != $tablet_top_padding
			...
			$css .= '@media (max-width: 768px){body .is-sticky #site-header.fixed-scroll #site-header-inner{padding:'. sunio_spacing_css( $tablet_top_padding, $tablet_right_padding, $tablet_bottom_padding, $tablet_left_padding ) .'}}';
		}
		...
		if ( isset( $mobile_top_padding ) && '' != $mobile_top_padding
			...
			$css .= '@media (max-width: 480px){body .is-sticky #site-header.fixed-scroll #site-header-inner{padding:'. sunio_spacing_css( $mobile_top_padding, $mobile_right_padding, $mobile_bottom_padding, $mobile_left_padding ) .'}}';
		}

		// Add opacity
		if ( ! empty( $opacity ) && '0.95' != $opacity ) {
			$css .= '.is-sticky #site-header,.sunio-sticky-top-bar-holder.is-sticky #top-bar-wrap,.is-sticky .header-top{opacity:'. $opacity .';}';
		}
		...
		if ( ! empty( $links_color ) ) {
			$css .= '.is-sticky #site-navigation-wrap .dropdown-menu > li > a,.is-sticky .sunio-mobile-menu-icon a,.is-sticky #searchform-header-replace-close{color:'. $links_color .';}';
		}

		// Add links hover color
		if ( ! empty( $links_hover_color ) ) {
			$css .= '.is-sticky #site-navigation-wrap .dropdown-menu > li > a:hover,.is-sticky .sunio-mobile-menu-icon a:hover,.is-sticky #searchform-header-replace-close:hover{color:'. $links_hover_color .';}';
		}
		...
		if ( ! empty( $social_links_color ) ) {
			$css .= '.is-sticky .sunio-social-menu ul li a,.is-sticky #site-header.full_screen-header .sunio-social-menu.simple-social ul li a{color:'. $social_links_color .';}';
		}

		// Menu social links hover color
		if ( ! empty( $social_hover_links_color ) ) {
			$css .= '.is-sticky .sunio-social-menu ul li a:hover,.is-sticky #site-header.full_screen-header .sunio-social-menu.simple-social ul li a:hover{color:'. $social_hover_links_color .'!important;}';
		}
