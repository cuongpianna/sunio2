 ...
 * @since 1.1.1
 */
if ( ! function_exists( 'sunio_logo_shortcode' ) ) {

	function sunio_logo_shortcode( $atts ) {
...
add_shortcode( 'sunio_logo', 'sunio_logo_shortcode' );
 ...
 * @since 1.1.1
 */
if ( ! function_exists( 'sunio_nav_shortcode' ) ) {

	function sunio_nav_shortcode( $atts ) {
...
add_shortcode( 'sunio_nav', 'sunio_nav_shortcode' );
 ...
 * @since 1.1.1
 */
if ( ! function_exists( 'sunio_date_shortcode' ) ) {

	function sunio_date_shortcode( $atts ) {
...
add_shortcode( 'sunio_date', 'sunio_date_shortcode' );
 ...
 * @since 1.1.9
 */
if ( ! function_exists( 'sunio_search_shortcode' ) ) {

	function sunio_search_shortcode( $atts ) {

		// Extract attributes
		extract( shortcode_atts( array(
			'width' 		=> '',
			'height' 		=> '',
			'placeholder' 	=> esc_html__( 'Search', 'sunio-extra' ),
		...
		$html = '<form method="get" class="sunio-searchform" id="searchform" action="'. esc_url( home_url( '/' ) ) .'"'. $style .'>';
...
add_shortcode( 'sunio_search', 'sunio_search_shortcode' );
 ...
 * @since 1.1.9
 */
if ( ! function_exists( 'sunio_site_url_shortcode' ) ) {

	function sunio_site_url_shortcode( $atts ) {
...
add_shortcode( 'sunio_site_url', 'sunio_site_url_shortcode' );
 ...
 * @since 1.1.9
 */
if ( ! function_exists( 'sunio_login_shortcode' ) ) {

	function sunio_login_shortcode( $atts ) {

		extract( shortcode_atts( array(
			'custom_url' 		=> '',
			'login_text' 		=> esc_html__( 'Login', 'sunio-extra' ),
			'logout_text' 		=> esc_html__( 'Log Out', 'sunio-extra' ),
			'target' 			=> 'self',
			'logout_redirect' 	=> '',
		), $atts ) );
		...
		if ( is_user_logged_in() ) {
			return '<a href="'. esc_url( $logout_url ) .'" title="'. esc_attr( $logout_text ) .'" class="sunio-logout">'. strip_tags( $logout_text ) .'</a>';
		}

		// Logged out link
		else {
			return '<a href="'. esc_url( $login_url ) .'" title="'. esc_attr( $login_text ) .'" class="sunio-login" target="_'. esc_attr( $target ) .'">'. strip_tags( $login_text ) .'</a>';
		}

	}

}
add_shortcode( 'sunio_login', 'sunio_login_shortcode' );
 ...
 * @since 1.2.1
 */
if ( ! function_exists( 'sunio_current_user_shortcode' ) ) {

	function sunio_current_user_shortcode( $atts ) {

		extract( shortcode_atts( array(
			'text' 			=> esc_html__( 'Welcome back', 'sunio-extra' ),
...
add_shortcode( 'sunio_current_user', 'sunio_current_user_shortcode' );
 ...
 * @since 1.2.2
 */
if ( ! function_exists( 'sunio_woo_fragments' ) ) {

	function sunio_woo_fragments( $fragments ) {
		...
		$fragments['.sunio-woo-total'] 			= '<span class="sunio-woo-total">' . WC()->cart->get_total() . '</span>';
	    $fragments['.sunio-woo-cart-count'] 		= '<span class="sunio-woo-cart-count">' . WC()->cart->get_cart_contents_count() . '</span>';
	    return $fragments;
	}

}
add_filter( 'woocommerce_add_to_cart_fragments', 'sunio_woo_fragments', 10, 1 );
 ...
 * @since 1.4.4
 */
if ( ! function_exists( 'sunio_woo_cart_icon_shortcode' ) ) {

	function sunio_woo_cart_icon_shortcode( $atts ) {
		...
		if ( class_exists( 'Elementor\Plugin' )
			...
			return esc_html__( 'This shortcode only works in front end', 'sunio-extra' );
		}
		...
		if ( ! empty( $color )
			...
			// Add style
			if ( ! empty( $css ) ) {
				echo "<style type=\"text/css\">\n" . wp_strip_all_tags( sunio_minify_css( $css ) ) . "\n</style>";
			}
...
add_shortcode( 'sunio_woo_cart', 'sunio_woo_cart_icon_shortcode' );
 ...
 * @since 1.2.2
 */
if ( ! function_exists( 'sunio_woo_total_cart_shortcode' ) ) {

	function sunio_woo_total_cart_shortcode() {
		...
		$html  = '<span class="sunio-woo-total">';
...
add_shortcode( 'sunio_woo_total_cart', 'sunio_woo_total_cart_shortcode' );
 ...
 * @since 1.2.2
 */
if ( ! function_exists( 'sunio_woo_cart_items_shortcode' ) ) {

	function sunio_woo_cart_items_shortcode() {
		...
		$html  = '<span class="sunio-woo-cart-count">';
...
add_shortcode( 'sunio_woo_cart_items', 'sunio_woo_cart_items_shortcode' );
 ...
 * @since 1.2.2
 */
if ( ! function_exists( 'sunio_woo_free_shipping_left' ) ) {

	function sunio_woo_free_shipping_left( $content, $content_reached, $multiply_by = 1 ) {
		...
		if ( empty( $content ) ) {
			$content = esc_html__( 'Buy for %left_to_free% more and get free shipping', 'sunio-extra' );
		}

		if ( empty( $content_reached ) ) {
			$content_reached = esc_html__( 'You have Free delivery!', 'sunio-extra' );
		}
		...
		if ( 0 != $min_free_shipping_amount ) {
			if ( isset( WC()->cart->cart_contents_total ) ) {
				...
				if ( $total >= $min_free_shipping_amount ) {
					...
					$content = str_replace( '%left_to_free%',             '<span class="sunio-woo-left-to-free">'. wc_price( ( $min_free_shipping_amount - $total ) * $multiply_by ) .'</span>', $content );
					$content = str_replace( '%free_shipping_min_amount%', '<span class="sunio-woo-left-to-free">'. wc_price( ( $min_free_shipping_amount )          * $multiply_by ) .'</span>', $content );
					return $content;
				}
			}
		}
...
if ( ! function_exists( 'sunio_woo_free_shipping_left_shortcode' ) ) {

	function sunio_woo_free_shipping_left_shortcode( $atts, $content ) {
        ...
        return sunio_woo_free_shipping_left( "<span class='sunio-woo-free-shipping' data-content='$x' data-reach='$content_reached'>". $content .'</span>', '<span class="sunio-woo-free-shipping">'. $content_reached .'</span>', $multiply_by );

	}

}
add_shortcode( 'sunio_woo_free_shipping_left', 'sunio_woo_free_shipping_left_shortcode' );
 ...
 * @since 1.4.24
 */
if ( ! function_exists( 'update_sunio_woo_free_shipping_left_shortcode' ) ) {

	function update_sunio_woo_free_shipping_left_shortcode() {
	    $atts = array();
	    ...
	    if ( ( isset( $_POST['content'] )
	        ...
	        $returnShortCodeValue 		= sunio_woo_free_shipping_left_shortcode( $atts, '' );
...
add_action( 'wp_ajax_update_sunio_woo_free_shipping_left_shortcode', 'update_sunio_woo_free_shipping_left_shortcode' );
add_action( 'wp_ajax_nopriv_update_sunio_woo_free_shipping_left_shortcode', 'update_sunio_woo_free_shipping_left_shortcode' );
 ...
 * @since 1.4.24
 */
function sunio_woo_free_shipping_left_script() {
	wp_register_script( 'owp-free-shipping', plugins_url( '/js/shortcode.min.js', __FILE__ ), false, true );
}
add_action( 'wp_enqueue_scripts', 'sunio_woo_free_shipping_left_script' );
 ...
 * @since 1.3.3
 */
if ( ! function_exists( 'sunio_breadcrumb_shortcode' ) ) {

	function sunio_breadcrumb_shortcode( $atts ) {
		...
		// Return if sunio_Breadcrumb_Trail doesn't exist
		if ( ! class_exists( 'sunio_Breadcrumb_Trail' ) ) {
			return;
		}
		...
		if ( ! empty( $color ) || ! empty( $hover_color ) ) {
			...
			if ( ! empty( $color ) ) {
				$css .= '.sunio-breadcrumb .site-breadcrumbs, .sunio-breadcrumb .site-breadcrumbs a {color:'. $color .';}';
			}

			if ( ! empty( $hover_color ) ) {
				$css .= '.sunio-breadcrumb .site-breadcrumbs a:hover {color:'. $hover_color .';}';
			}
		...
		if ( function_exists( 'yoast_breadcrumb' ) && current_theme_supports( 'yoast-seo-breadcrumbs' ) ) {
			...
			if ( $breadcrumbs_position = get_theme_mod( 'sunio_breadcrumbs_position' ) ) {
		...
		if ( ! is_object( $breadcrumb ) ) {
			$breadcrumb = new sunio_Breadcrumb_Trail( $args );
		}

		return '<span class="sunio-breadcrumb'. $class .'">'. $breadcrumb->get_trail() .'</span>';

	}

}
add_shortcode( 'sunio_breadcrumb', 'sunio_breadcrumb_shortcode' );