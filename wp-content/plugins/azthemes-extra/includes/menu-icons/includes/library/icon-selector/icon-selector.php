<?php
final class AZT_Icon_Selector {

	const VERSION = '1.0.0';

	/**
	 * AZT_Icon_Selector singleton
	 * 
	 */
	...
	protected function __construct( $args = array() ) {
		$defaults = array(
			'dir' => AZT_PATH .'includes/menu-icons/includes/library/icon-selector',
			'url' => AZT_URL .'includes/menu-icons/includes/library/icon-selector',
		);
	...
	protected function init_registry() {
		...
		$this->registry = AZT_Icon_Picker_Types_Registry::instance();
	...
	protected function register_default_types() {
		...
		AZT_Icon_Picker_Fontpack::instance();
		...
		foreach ( $default_types as $filename => $class_suffix ) {
			$class_name = "AZT_Icon_Picker_Type_{$class_suffix}";
	...
	protected function init_loader() {
		...
		$this->loader = AZT_Icon_Picker_Loader::instance();
...
add_action( 'wp_loaded', array( 'AZT_Icon_Selector', 'instance' ), 7 );
