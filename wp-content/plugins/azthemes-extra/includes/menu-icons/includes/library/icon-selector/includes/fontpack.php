...
final class AZT_Icon_Picker_Fontpack {

	/**
	 * AZT_Icon_Picker_Fontpack singleton
	 *
	 */
	...
	protected function __construct() {
		...
		$this->messages = array(
			'no_config'    => __( 'Icon Picker: %1$s was not found in %2$s.', 'sunio-extra' ),
			'config_error' => __( 'Icon Picker: %s contains an error or more.', 'sunio-extra' ),
			'invalid'      => __( 'Icon Picker: %1$s is not set or invalid in %2$s.', 'sunio-extra' ),
			'duplicate'    => __( 'Icon Picker: %1$s is already registered. Please check your font pack config file: %2$s.', 'sunio-extra' ),
		);
	...
	protected function register_packs() {
		...
		$icon_picker = AZT_Icon_Selector::instance();
		require_once "{$icon_picker->dir}/includes/types/fontello.php";

		foreach ( $this->packs as $pack_data ) {
			$icon_picker->registry->add( new AZT_Icon_Picker_Type_Fontello( $pack_data ) );
		}
	}
	...
	protected function get_pack_data( DirectoryIterator $pack_dir ) {
		...
		$pack_data = array(
			'id'             => "pack-{$config['name']}",
			'name'           => sprintf( __( 'Pack: %s', 'sunio-extra' ), $config['name'] ),
