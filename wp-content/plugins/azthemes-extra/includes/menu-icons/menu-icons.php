...
final class AZT_Menu_Icons {
	...
	public static function _load() {
		...
		self::$data = array(
			'dir'   => AZT_PATH .'includes/menu-icons/',
			'url'   => AZT_URL .'includes/menu-icons/',
			'types' => array(),
		);

		// Load Icon Picker.
		if ( ! class_exists( 'AZT_Icon_Selector' ) ) {
	...
	public static function _init() {
		...
		self::$data['types'] = apply_filters(
			'oe_menu_icons_types',
			AZT_Icon_Picker_Types_Registry::instance()->types
		);

		// Nothing to do if there are no icon types registered.
		if ( empty( self::$data['types'] ) ) {
			if ( WP_DEBUG ) {
				trigger_error( esc_html__( 'Menu Icons: No registered icon types found.', 'sunio-extra' ) );
			}
	...
	public static function _notice_missing_icon_picker() {
		?>
		<div class="error">
			<p><?php esc_html_e( 'Please activate Icon Picker first.', 'sunio-extra' ); ?></p>
		</div>
		<?php
	}
}
add_action( 'init', array( 'AZT_Menu_Icons', '_load' ) );
