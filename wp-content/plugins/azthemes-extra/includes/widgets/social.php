...
if ( ! class_exists( 'sunio_Extra_Social_Widget' ) ) {
	class sunio_Extra_Social_Widget extends WP_Widget {
		...
		public function __construct() {
			...
			$this->social_services_array = apply_filters( 'sunio_social_widget_profiles', array(
				'twitter' => array(
					'name' => 'Twitter',
					'url'  => '',
				),
			...
			parent::__construct(
				'sunio_social',
				esc_html__( '&raquo; Social Icons', 'sunio-extra' ),
				array(
					'classname'   => 'widget-sunio-social social-widget',
					'description' => esc_html__( 'Display your social media icons.', 'sunio-extra' ),
		...
		public function social_widget_style() { ?>
			<style>
				.sunio-social-widget-services-list { padding-top: 10px; }
				.sunio-social-widget-services-list li { cursor: move; background: #fafafa; padding: 10px; border: 1px solid #e5e5e5; margin-bottom: 10px; }
				.sunio-social-widget-services-list li p { margin: 0 }
				.sunio-social-widget-services-list li label { margin-bottom: 3px; display: block; color: #222; }
				.sunio-social-widget-services-list li label span.fa { margin-right: 10px }
				.sunio-social-widget-services-list .placeholder { border: 1px dashed #e3e3e3 }
				.sunio-widget-select { width: 100% }
			...
			<script>
				( function( $ ){
					$(document).ajaxSuccess(function(e, xhr, settings) {
						var widget_id_base = 'sunio_social';
						if ( settings.data.search( 'action=save-widget' ) != -1 && settings.data.search( 'id_base=' + widget_id_base) != -1 ) {
							sunioSortServices();
						}
					} );

					function sunioSortServices() {
						$( '.sunio-social-widget-services-list' ).each( function() {
							var id = $(this).attr( 'id' );
							$( '#'+ id ).sortable( {
		...
		public function widget( $args, $instance ) {
			...
			echo $args['before_widget'];
				...
				<ul class="sunio-social-icons <?php echo esc_attr( $transition ); ?> style-<?php echo esc_attr( $style ); ?>">
					<?php
					// Original Array
					...
					foreach( $social_services as $key => $val ) {
						...
						if ( $link ) {
							...
							echo '<li class="sunio-'. esc_attr( $key ) .'">';
		...
		public function form( $instance ) {
			...
			$instance = wp_parse_args( ( array ) $instance, array(
				'title'           	 => esc_attr__( 'Follow Us', 'sunio-extra' ),
				'style' 	  		 => esc_html__( 'Light', 'sunio-extra' ),
				'transition' 	  	 => esc_html__( 'Rotate', 'sunio-extra' ),
				'font_size'       	 => '',
				'border_radius'   	 => '',
				'bg_color'   	     => '',
				'bg_hover_color'   	 => '',
			...
			<p>
				...
				<select class='widefat' name="<?php echo esc_attr( $this->get_field_name( 'style' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'style' ) ); ?>">
					<option value="light" <?php selected( $instance['style'], 'light' ) ?>><?php esc_html_e( 'Light', 'sunio-extra' ); ?></option>
					<option value="dark" <?php selected( $instance['style'], 'dark' ) ?>><?php esc_html_e( 'Dark', 'sunio-extra' ); ?></option>
					<option value="colored" <?php selected( $instance['style'], 'colored' ) ?>><?php esc_html_e( 'Colored', 'sunio-extra' ); ?></option>
					<option value="simple" <?php selected( $instance['style'], 'simple' ) ?>><?php esc_html_e( 'Simple', 'sunio-extra' ); ?></option>
				</select>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'transition' ) ); ?>"><?php esc_html_e( 'Transition:', 'sunio-extra' ); ?></label>
				<select class='widefat' name="<?php echo esc_attr( $this->get_field_name( 'transition' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'transition' ) ); ?>">
					<option value="no-transition" <?php selected( $instance['transition'], 'no-transition' ) ?>><?php esc_html_e( 'None', 'sunio-extra' ); ?></option>
					<option value="float" <?php selected( $instance['transition'], 'float' ) ?>><?php esc_html_e( 'Float', 'sunio-extra' ); ?></option>
					<option value="rotate" <?php selected( $instance['transition'], 'rotate' ) ?>><?php esc_html_e( 'Rotate', 'sunio-extra' ); ?></option>
					<option value="zoomout" <?php selected( $instance['transition'], 'zoomout' ) ?>><?php esc_html_e( 'Zoom Out', 'sunio-extra' ); ?></option>
				</select>
			</p>
			
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>"><?php esc_html_e( 'Link Target', 'sunio-extra' ); ?>:</label>
				<br />
				<select class="sunio-widget-select" name="<?php echo esc_attr( $this->get_field_name( 'target' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>">
					<option value="blank" <?php selected( $instance['target'], 'blank' ) ?>><?php esc_html_e( 'Blank', 'sunio-extra' ); ?></option>
					<option value="self" <?php selected( $instance['target'], 'self' ) ?>><?php esc_html_e( 'Self', 'sunio-extra' ); ?></option>
				</select>
			</p>
			
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'size' ) ); ?>"><?php esc_html_e( 'Dimensions', 'sunio-extra' ); ?>:</label>
				...
				<small><?php esc_html_e( 'Example:', 'sunio-extra' ); ?> 40px</small>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'font_size' ) ); ?>"><?php esc_html_e( 'Font Size', 'sunio-extra' ); ?>:</label>
				...
				<small><?php esc_html_e( 'Example:', 'sunio-extra' ); ?> 18px</small>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'border_radius' ) ); ?>"><?php esc_html_e( 'Border Radius', 'sunio-extra' ); ?></label>
				...
				<small><?php esc_html_e( 'Example:', 'sunio-extra' ); ?> 4px</small>
			</p>

			<p>
				<label class="color-label" for="<?php echo esc_attr( $this->get_field_id( 'bg_color' ) ); ?>"><?php esc_html_e( 'Background Color', 'sunio-extra' ); ?></label>
			...
			<p>
				<label class="color-label" for="<?php echo esc_attr( $this->get_field_id( 'bg_hover_color' ) ); ?>"><?php esc_html_e( 'Background Color Hover', 'sunio-extra' ); ?></label>
			...
			<p>
				<label class="color-label" for="<?php echo esc_attr( $this->get_field_id( 'color' ) ); ?>"><?php esc_html_e( 'Color', 'sunio-extra' ); ?></label>
			...
			<p>
				<label class="color-label" for="<?php echo esc_attr( $this->get_field_id( 'color_hover' ) ); ?>"><?php esc_html_e( 'Color Hover', 'sunio-extra' ); ?></label>
			...
			<p>
				<label class="color-label" for="<?php echo esc_attr( $this->get_field_id( 'border_color' ) ); ?>"><?php esc_html_e( 'Border Color', 'sunio-extra' ); ?></label>
			...
			<p>
				<label class="color-label" for="<?php echo esc_attr( $this->get_field_id( 'border_hover_color' ) ); ?>"><?php esc_html_e( 'Border Color Hover', 'sunio-extra' ); ?></label>
			...
			<h3 style="margin-top:20px;margin-bottom:0;"><?php esc_html_e( 'Social Links','sunio-extra' ); ?></h3>
			<ul id="<?php echo esc_attr( $field_id_services ); ?>" class="sunio-social-widget-services-list">
				<input type="hidden" id="<?php echo esc_attr( $field_name_services ); ?>" value="<?php echo esc_attr( $field_name_services ); ?>">
				<input type="hidden" id="<?php echo esc_attr( wp_create_nonce( 'sunio_fontawesome_social_widget_nonce' ) ); ?>">
				<?php
				// Social array
				$social_services_array = $this->social_services_array;
		...
		public function colors( $args, $instance ) {
			...
			if ( $bg_color || $bg_hover_color
				...
				<style>
					#<?php echo $id; ?>.widget-sunio-social ul li a {
					...
					#<?php echo $id; ?>.widget-sunio-social ul li a:hover {
...
register_widget( 'sunio_Extra_Social_Widget' );