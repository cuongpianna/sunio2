...
// Start class
if ( ! class_exists( 'sunio_Extra_About_Me_Widget' ) ) {
	class sunio_Extra_About_Me_Widget extends WP_Widget {
		...
		public function __construct() {
			...
			$this->social_services_array = apply_filters( 'sunio_about_me_widget_profiles', array(
				'facebook'		=> array(
					'name'		=> 'Facebook',
					'url'		=> ''
				),
			...
			parent::__construct(
				'sunio_about_me',
				esc_html__( '&raquo; About Me', 'sunio-extra' ),
				array(
					'classname'   => 'widget-sunio-about-me about-me-widget',
					'description' => esc_html__( 'Adds a about me widget.', 'sunio-extra' ),
		...
		public function social_widget_style() { ?>
			<style>
				.sunio-about-me-widget-services-list { padding-top: 10px; }
				.sunio-about-me-widget-services-list li { cursor: move; background: #fafafa; padding: 10px; border: 1px solid #e5e5e5; margin-bottom: 10px; }
				.sunio-about-me-widget-services-list li p { margin: 0 }
				.sunio-about-me-widget-services-list li label { margin-bottom: 3px; display: block; color: #222; }
				.sunio-about-me-widget-services-list li label span.fa { margin-right: 10px }
				.sunio-about-me-widget-services-list .placeholder { border: 1px dashed #e3e3e3 }
			...
			<script>
				( function( $ ){
					$(document).ajaxSuccess(function(e, xhr, settings) {
						var widget_id_base = 'sunio_about_me';
					...
					function sunioSortServices() {
						$( '.sunio-about-me-widget-services-list' ).each( function() {
							var id = $(this).attr( 'id' );
							$( '#'+ id ).sortable( {
		...
		public function widget( $args, $instance ) {
			...
			echo $args['before_widget'];
				...
				<div class="sunio-about-me">

					<div class="sunio-about-me-avatar clr">
						...
						// Display the name
						if ( $name ) :?>
							<h3 class="sunio-about-me-name"><?php echo esc_html( $name ); ?></h3>
						<?php endif; ?>

					</div><!-- .sunio-about-me-avatar -->

					<?php
					// Display the text
					if ( $text ) : ?>
						<div class="sunio-about-me-text clr"><?php echo do_shortcode( $text ); ?></div>
					<?php endif;

					// Display the social
					if ( $social_services ) : ?>

						<ul class="sunio-about-me-social style-<?php echo esc_attr( $social_style ); ?>">
							<?php
							// Original Array
		...
		public function form( $instance ) {
			$instance = wp_parse_args( ( array ) $instance, array(
				'title'       		=> esc_html__( 'About Me', 'sunio-extra' ),
				'avatar' 			=> AZT_URL .'assets/img/about-avatar.png',
				'name'  			=> esc_html__( 'John Doe', 'sunio-extra' ),
			...
			<p>
				...
				<input class="sunio-widget-upload-button button button-secondary" type="button" value="<?php esc_html_e( 'Upload Image', 'sunio-extra' ); ?>" />
			</p>

			<p>
			    <label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e('Name:', 'sunio-extra') ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php esc_html_e( 'Description:','sunio-extra' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id('social_style') ); ?>"><?php esc_html_e('Social Style:', 'sunio-extra'); ?></label>
				<br />
				<select class='widget-select widefat' name="<?php echo esc_attr( $this->get_field_name('social_style') ); ?>" id="<?php echo esc_attr( $this->get_field_id('social_style') ); ?>">
					<option value="color" <?php selected( $instance['social_style'], 'color' ) ?>><?php esc_html_e( 'Color', 'sunio-extra' ); ?></option>
					<option value="light" <?php selected( $instance['social_style'], 'light' ) ?>><?php esc_html_e( 'Light', 'sunio-extra' ); ?></option>
					<option value="dark" <?php selected( $instance['social_style'], 'dark' ) ?>><?php esc_html_e( 'Dark', 'sunio-extra' ); ?></option>
				</select>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id('target') ); ?>"><?php esc_html_e( 'Social Link Target:', 'sunio-extra' ); ?></label>
				<br />
			...
			<ul id="<?php echo esc_attr( $field_id_services ); ?>" class="sunio-about-me-widget-services-list">
			...
			<script type="text/javascript">
				(function($) {
					...
					$( document ).ready( function() {
						...
						$( '.sunio-widget-upload-button' ).click(function(e) {
...
register_widget( 'sunio_Extra_About_Me_Widget' );