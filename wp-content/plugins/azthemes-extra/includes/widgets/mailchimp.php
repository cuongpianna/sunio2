...
if (!class_exists('sunio_Extra_MailChimp_Widget')) {

    class sunio_Extra_MailChimp_Widget extends WP_Widget {
        ...
        public function __construct() {
            parent::__construct(
                    'sunio_mailchimp', esc_html__('&raquo; MailChimp', 'sunio-extra'), array(
                'classname' => 'widget-sunio-mailchimp mailchimp-widget',
                'description' => esc_html__('Displays mailchimp subscription form.', 'sunio-extra'),
                'customize_selective_refresh' => true,
                    )
            );

            add_action( 'wp_enqueue_scripts', array( $this, 'sunio_extra_mailchimp_js' ) );
            add_filter( 'sunio_localize_array', array( $this, 'localize_array' ) );

            add_action('wp_ajax_sunio_mailchimp_request', array($this, 'sunio_mailchimp_request_callback'));
            add_action('wp_ajax_nopriv_sunio_mailchimp_request', array($this, 'sunio_mailchimp_request_callback'));
        }

        public function sunio_mailchimp_request_callback() {
        ...
        public function widget($args, $instance) {
            ...
            <div class="sunio-newsletter-form clr">

                <div class="sunio-newsletter-form-wrap">

                    <?php if ($subscribe_text) { ?>

                        <div class="sunio-mail-text"><?php echo do_shortcode($subscribe_text); ?></div>
                    ...
                    <form action="" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate<?php echo wp_kses_post($form_style); ?>>
                        ...
                        <span class="email-err err-msg req" style="display:none;"><?php _e("Email is required.", "sunio-extra"); ?></span>
                        <span class="email-err err-msg not-valid" style="display:none;"><?php _e("Email not valid.", "sunio-extra"); ?></span>
                        ...
                        <?php if ($mailchimp_gdpr_label) { ?>
                            <div class="gdpr-wrap elem-wrap">
                                ...
                                <span class="gdpr-err err-msg" style="display:none;"><?php _e("This field is required", "sunio-extra"); ?></span>
                            </div>
                        <?php } ?>

                        <div class="success res-msg" style="display:none;"><?php _e("Thanks for your subscription.", "sunio-extra"); ?></div>
                        <div class="failed  res-msg" style="display:none;"><?php _e("Failed to subscribe, please contact admin.", "sunio-extra"); ?></div>
                    </form>

                </div><!--.sunio-newsletter-form-wrap-->

            </div><!-- .sunio-newsletter-form -->
        ...
        public function form($instance) {
            ...
            $instance = wp_parse_args((array) $instance, array(
                'title' => esc_attr__('Newsletter', 'sunio-extra'),
                'subscribe_text' => esc_html__('Get all latest content delivered to your email a few times a month. Updates and news about all categories will send to you.', 'sunio-extra'),
                'mailchimp_gdpr_label' => esc_attr__('Accept GDPR Terms', 'sunio-extra'),
                'width' => '',
                'height' => '',
                'placeholder' => esc_html__('Your Email', 'sunio-extra'),
                'submit_text' => esc_html__('Go', 'sunio-extra'),
            ));
            ...
            if ( ! get_option( 'owp_mailchimp_api_key' )
                ...
                <p>
                    <?php echo sprintf(
                        __( 'You need to set your Api Key & List Id on the %1$ssettings page%2$s', 'sunio-extra' ),
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('subscribe_text')); ?>">
                    <?php esc_html_e('Text', 'sunio-extra'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('mailchimp_gdpr_label')); ?>"><?php esc_html_e('GDPR Field Label', 'sunio-extra'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('width')); ?>"><?php esc_html_e('Input Width (px)', 'sunio-extra'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('height')); ?>"><?php esc_html_e('Input Height (px)', 'sunio-extra'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('placeholder')); ?>"><?php esc_html_e('Placeholder', 'sunio-extra'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('submit_text')); ?>"><?php esc_html_e('Submit Text', 'sunio-extra'); ?></label>
         ...
         */
        public function sunio_extra_mailchimp_js() {
            // Load only if the widget is used
            if ( is_active_widget( '', '', 'sunio_mailchimp' ) ) {
                wp_enqueue_script('oe-mailchimp-script', AZT_URL . 'includes/widgets/js/mailchimp.min.js', array('jquery'), false, true);
            }
        }
...
register_widget('sunio_Extra_MailChimp_Widget');
