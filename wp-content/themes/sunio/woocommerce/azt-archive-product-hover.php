...
echo '<ul class="woo-entry-inner clr">';
	...
	echo '<li class="image-wrap">';
		...
		if ( get_theme_mod( 'sunio_woo_quick_view', true )
			...
			echo '<ul class="woo-entry-buttons">';
				do_action( 'sunio_before_archive_woo_entry_buttons' );
				if ( get_theme_mod( 'sunio_woo_quick_view', true ) ) {
					echo '<li class="woo-quickview-btn">'. apply_filters( 'sunio_woo_quick_view_button_html', '<a href="#" class="azt-quick-view" id="product_id_' . $product->get_id() . '" data-product_id="' . $product->get_id() . '"><i class="icon-eye"></i></a>' ) .'</li>';
				}

				if ( class_exists( 'TInvWL_Wishlist' ) ) {
					echo '<li class="woo-wishlist-btn">'. do_shortcode( '[ti_wishlists_addtowishlist]' ) .'</li>';
				}
				do_action( 'sunio_after_archive_woo_entry_buttons' );
			echo '</ul>';
		}
	...
	echo '<ul class="woo-product-info">';
		...
		echo '<li class="price-wrap">';
			...
			do_action( 'sunio_after_archive_product_price' );
		echo '</li>';
	...
	if ( function_exists( 'wc_get_template' ) ) {
		wc_get_template( 'azt-archive-product-thumbnails.php' );
	}
