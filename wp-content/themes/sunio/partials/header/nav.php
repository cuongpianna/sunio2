...
// Get ID
$template = sunio_custom_nav_template();

// Check if page is Elementor page
$elementor  = get_post_meta( $template, '_elementor_edit_mode', true );

// Get content
$get_content = sunio_nav_template_content();
...
if ( ! empty( $template ) ) {
	...
	<?php } ?>
	    ...
	    if ( sunio_ELEMENTOR_ACTIVE && $elementor ) {
	        ...
	        sunio_Elementor::get_nav_content();
...
<?php
	...
	if ( has_nav_menu( $menu_location ) || $ms_global_menu ) :
		...
		if ( $menu = sunio_header_custom_menu() ) {
			$menu_args['menu']  = $menu;
		}
		...
		<?php } ?>
			...
			<nav id="site-navigation" class="<?php echo esc_attr( $inner_classes ); ?>"<?php sunio_schema_markup( 'site_navigation' ); ?>>
				...
				if ( 'top' != $header_style
					...
					} else if ( 'header_replace' == sunio_menu_search_style() ) {
				...
				if ( 'full_screen' == $header_style
					&& true == get_theme_mod( 'sunio_menu_social', false ) ) {
