...
$position = get_theme_mod( 'sunio_center_header_menu_position', 'centered' );
...
// Left menu
$left_menu = get_theme_mod( 'sunio_center_header_left_menu' );
$left_menu = '0' != $left_menu ? $left_menu : '';
$left_menu = apply_filters( 'sunio_center_header_left_menu', $left_menu );

// Right menu
$right_menu = apply_filters( 'sunio_main_menu_location', 'main_menu' );
$right_custom_menu = apply_filters( 'sunio_custom_menu', $right_menu );
...
<div id="site-header-inner" class="<?php echo esc_attr( $classes ); ?>">
	...
	<?php do_action( 'sunio_header_inner_left_content' ); ?>
	...
	$wrap_classes  = sunio_header_menu_classes( 'wrapper' );
	$inner_classes = sunio_header_menu_classes( 'inner' );
	...
	$left_menu_args = array(
		...
		'link_after'     => '</span>',
		'walker'         => new sunio_Custom_Nav_Walker(),
	);
	...
	do_action( 'sunio_before_nav' ); ?>

	<div id="site-navigation-wrap" class="<?php echo esc_attr( $wrap_classes ); ?>">

		<?php do_action( 'sunio_before_nav_inner' ); ?>

		<nav id="site-navigation" class="<?php echo esc_attr( $inner_classes ); ?>"<?php sunio_schema_markup( 'site_navigation' ); ?>>
			...
			<div class="middle-site-logo <?php echo esc_attr( sunio_header_logo_classes() ); ?>">
			...
			<ul class="right-menu <?php echo esc_attr( $menu_classes ); ?>">
				...
				if ( has_nav_menu( $right_menu ) ) {
						...
						// Drop down search
						if ( 'drop_down' == sunio_menu_search_style() ) {
							get_template_part( 'partials/header/search-dropdown' );
						}

						// WooCommerce cart
						if ( 'drop_down' == sunio_menu_cart_style() ) {
		...
		<?php do_action( 'sunio_after_nav_inner' ); ?>

	</div><!-- #site-navigation-wrap -->

	<?php do_action( 'sunio_header_inner_right_content' ); ?>
