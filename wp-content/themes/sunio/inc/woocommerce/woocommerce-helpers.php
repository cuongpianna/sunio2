 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'sunio_wcmenucart_menu_item' ) ) {

	function sunio_wcmenucart_menu_item() {

		// Classes
		$classes = array( 'wcmenucart' );

		// Hide items if "hide if empty cart" is checked
		if ( true == get_theme_mod( 'sunio_woo_menu_icon_hide_if_empty', false )
		...
		// Vars
		$icon_style   = get_theme_mod( 'sunio_woo_menu_icon_style', 'drop_down' );
		$custom_link  = get_theme_mod( 'sunio_woo_menu_icon_custom_link' );
		...
		// Cart total
		$display = get_theme_mod( 'sunio_woo_menu_icon_display', 'icon_count' );
		...
		// Get cart icon
		$icon = get_theme_mod( 'sunio_woo_menu_icon', 'icon-handbag' );
		...
		if ( 'yes' == get_theme_mod( 'sunio_woo_menu_bag_style', 'no' ) ) { ?>
			...
			<a href="<?php echo esc_url( $url ); ?>" class="<?php echo esc_attr( $classes ); ?>">
				<?php
				if ( true == get_theme_mod( 'sunio_woo_menu_bag_style_total', false ) ) { ?>
 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'sunio_woo_placeholder_img' ) ) {

	function sunio_woo_placeholder_img() {
		if ( function_exists( 'wc_placeholder_img_src' ) && wc_placeholder_img_src() ) {
			$placeholder = '<div class="woo-entry-image clr"><img src="'. wc_placeholder_img_src() .'" alt="'. esc_html__( 'Placeholder Image', 'sunio' ) .'" class="woo-entry-image-main" /></div>';
			$placeholder = apply_filters( 'sunio_woo_placeholder_img_html', $placeholder );
			if ( $placeholder ) {
 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'sunio_woo_product_instock' ) ) {

	function sunio_woo_product_instock( $post_id = '' ) {
		global $post;
 ...
 * @since 1.1.9
 */
if ( ! function_exists( 'sunio_woo_product_elements_positioning' ) ) {
	...
	function sunio_woo_product_elements_positioning() {
		...
		$sections = apply_filters( 'sunio_woo_product_elements_positioning', $sections );
 ...
 * @since 1.1.9
 */
if ( ! function_exists( 'sunio_woo_summary_elements_positioning' ) ) {
	...
	function sunio_woo_summary_elements_positioning() {
		...
		// Apply filters for easy modification
		$sections = apply_filters( 'sunio_woo_summary_elements_positioning', $sections );
