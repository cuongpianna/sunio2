...
if ( ! class_exists( 'sunio_WooCommerce_Config' ) ) {
	...
	class sunio_WooCommerce_Config {
		...
		public function __construct() {
			...
			require_once( sunio_INC_DIR .'woocommerce/woocommerce-helpers.php' );
			...
			if ( ! is_admin() ) {
				...
				add_filter( 'sunio_get_sidebar', array( $this, 'display_woo_sidebar' ) );

				// Set correct post layouts
				add_filter( 'sunio_post_layout_class', array( $this, 'layouts' ) );

				// Set correct both sidebars layout style
				add_filter( 'sunio_both_sidebars_style', array( $this, 'bs_class' ) );

				// Set correct mobile sidebar order
				add_filter( 'sunio_sidebar_order', array( $this, 'sidebar_order' ) );
			...
			if ( 'yes' != get_theme_mod( 'sunio_woo_remove_custom_features', 'no' ) ) {
				...
				// Define accents
				add_filter( 'sunio_primary_texts', array( $this, 'primary_texts' ) );
				add_filter( 'sunio_primary_borders', array( $this, 'primary_borders' ) );
				add_filter( 'sunio_primary_backgrounds', array( $this, 'primary_backgrounds' ) );
				add_filter( 'sunio_hover_primary_backgrounds', array( $this, 'hover_primary_backgrounds' ) );
				...
				if ( ! is_admin() ) {
					...
					add_filter( 'sunio_post_subheading', array( $this, 'post_subheading' ) );

					// Show/hide next/prev on products
					add_filter( 'sunio_has_next_prev', array( $this, 'next_prev' ) );

					// Border colors
					add_filter( 'sunio_border_color_elements', array( $this, 'border_color_elements' ) );

				}

				// Main Woo Actions
				add_action( 'wp_enqueue_scripts', array( $this, 'add_custom_scripts' ) );
				add_filter( 'sunio_localize_array', array( $this, 'localize_array' ) );
				if ( get_theme_mod( 'sunio_woo_shop_result_count', true )
					|| get_theme_mod( 'sunio_woo_shop_sort', true )
					|| get_theme_mod( 'sunio_woo_grid_list', true )
					|| true == get_theme_mod( 'sunio_woo_off_canvas_filter', false ) ) {
				...
				add_action( 'woocommerce_before_account_navigation', array( $this, 'sunio_before_account_navigation' ) );
				add_action( 'woocommerce_after_account_navigation', array( $this, 'sunio_after_account_navigation' ) );
				if ( get_option( 'woocommerce_enable_myaccount_registration' ) !== 'yes' ) {
					add_action('woocommerce_before_customer_login_form', array( $this, 'sunio_login_wrap_before' ) );
					add_action('woocommerce_after_customer_login_form', array( $this, 'sunio_login_wrap_after' ) );
				}
				if ( get_theme_mod( 'sunio_woo_category_image', 'no' ) == 'yes' ) {
				...
				if ( get_theme_mod( 'sunio_woo_quick_view', true ) ) {
					if ( 'hover' != get_theme_mod( 'sunio_woo_products_style', 'default' ) ) {
						if ( 'gallery-slider' == get_theme_mod( 'sunio_woo_product_entry_style', 'image-swap' ) ) {
							add_action( 'sunio_after_product_entry_slider', array( $this, 'quick_view_button' ) );
						} else {
							add_action( 'sunio_after_product_entry_image', array( $this, 'quick_view_button' ) );
						}
					}
					add_action( 'wp_ajax_sunio_product_quick_view', array( $this, 'product_quick_view_ajax' ) );
					add_action( 'wp_ajax_nopriv_sunio_product_quick_view', array( $this, 'product_quick_view_ajax' ) );
					add_action( 'wp_footer', array( $this, 'quick_view_template' ) );
					add_action( 'sunio_woo_quick_view_product_image', 'woocommerce_show_product_sale_flash', 10 );
					add_action( 'sunio_woo_quick_view_product_image', array( $this, 'quick_view_image' ), 20 );
					add_action( 'sunio_woo_quick_view_product_content', array( $this, 'single_product_content' ), 10 );
				}

				// Ajax single product add to cart
				add_action( 'wp_ajax_sunio_add_cart_single_product', array( $this, 'add_cart_single_product_ajax' ) );
				add_action( 'wp_ajax_nopriv_sunio_add_cart_single_product', array( $this, 'add_cart_single_product_ajax' ) );

				// Add cart overlay
				if ( 'yes' == get_theme_mod( 'sunio_woo_display_cart_product_added', 'no' ) ) {
					add_action( 'sunio_after_footer', array( $this, 'cart_overlay' ), 99 );
				}

				// Add mobile menu mini cart
				if ( get_theme_mod( 'sunio_woo_add_mobile_mini_cart', true ) ) {
				...
				if ( 'on' == get_theme_mod( 'sunio_woo_display_floating_bar', 'off' ) ) {
					...
					// Ajax add to cart
					add_action( 'wp_ajax_sunio_add_cart_floating_bar', array( $this, 'add_cart_floating_bar_ajax' ) );
					add_action( 'wp_ajax_nopriv_sunio_add_cart_floating_bar', array( $this, 'add_cart_floating_bar_ajax' ) );
				}
				...
				// Sale badge content
				if ( 'percent' == get_theme_mod( 'sunio_woo_sale_badge_content', 'sale' ) ) {
				...
				add_filter( 'sunio_display_top_bar', array( $this, 'distraction_free' ), 11 );
				add_filter( 'sunio_display_navigation', array( $this, 'distraction_free' ), 11 );
				...
				add_filter( 'sunio_display_page_header', array( $this, 'distraction_free' ), 11 );
				add_filter( 'sunio_display_footer_widgets', array( $this, 'distraction_free' ), 11 );
				add_filter( 'sunio_display_scroll_up_button', array( $this, 'distraction_free' ), 11 );
				...
				if ( true == get_theme_mod( 'sunio_woo_multi_step_checkout', false ) ) {
					...
					// Checkout validation
					add_action( 'wp_ajax_sunio_validate_checkout', array( $this, 'validate_checkout_callback' ) );
	            	add_action( 'wp_ajax_nopriv_sunio_validate_checkout', array( $this, 'validate_checkout_callback' ) );
		            ...
		            add_action( 'sunio_woocommerce_checkout_order_review', 'woocommerce_order_review', 10 );
	            	add_action( 'sunio_woocommerce_checkout_payment', 'woocommerce_checkout_payment', 20 );
		            add_action( 'sunio_checkout_login_form', array( $this, 'checkout_login_form' ), 10 );
		            add_action( 'sunio_woocommerce_checkout_coupon', 'woocommerce_checkout_coupon_form', 10 );
		...
		public static function woo_section( $wp_customize ) {
			$wp_customize->get_section( 'woocommerce_checkout' )->panel = 'sunio_woocommerce_panel';
			$wp_customize->get_section( 'woocommerce_store_notice' )->panel = 'sunio_woocommerce_panel';
			$wp_customize->get_section( 'woocommerce_product_images' )->panel = 'sunio_woocommerce_panel';
			$wp_customize->get_section( 'woocommerce_product_images' )->priority = 999;
			$wp_customize->get_control( 'woocommerce_shop_page_display' )->section = 'sunio_woocommerce_archives';
			$wp_customize->get_control( 'woocommerce_category_archive_display' )->section = 'sunio_woocommerce_archives';
			$wp_customize->get_control( 'woocommerce_default_catalog_orderby' )->section = 'sunio_woocommerce_archives';
			$wp_customize->get_control( 'woocommerce_checkout_company_field' )->section = 'sunio_woocommerce_checkout';
			$wp_customize->get_control( 'woocommerce_checkout_address_2_field' )->section = 'sunio_woocommerce_checkout';
			$wp_customize->get_control( 'woocommerce_checkout_phone_field' )->section = 'sunio_woocommerce_checkout';
			$wp_customize->get_control( 'woocommerce_checkout_highlight_required_fields' )->section = 'sunio_woocommerce_checkout';
			$wp_customize->get_control( 'wp_page_for_privacy_policy' )->section = 'sunio_woocommerce_checkout';
			$wp_customize->get_control( 'woocommerce_terms_page_id' )->section = 'sunio_woocommerce_checkout';
			$wp_customize->get_control( 'woocommerce_checkout_privacy_policy_text' )->section = 'sunio_woocommerce_checkout';
			$wp_customize->get_control( 'woocommerce_checkout_terms_and_conditions_checkbox_text' )->section = 'sunio_woocommerce_checkout';
		}
		...
		public static function register_woo_sidebar() {
			...
			if ( ! get_theme_mod( 'sunio_woo_custom_sidebar', true ) ) {
				return;
			}

			// Register new woo_sidebar widget area
			register_sidebar( array (
				'name'          => esc_html__( 'WooCommerce Sidebar', 'sunio' ),
				'id'            => 'woo_sidebar',
		...
		public static function layouts( $class ) {
			if ( sunio_is_woo_shop()
				|| sunio_is_woo_tax() ) {
				$class = get_theme_mod( 'sunio_woo_shop_layout', 'left-sidebar' );
			} elseif ( sunio_is_woo_single() ) {
				$class = get_theme_mod( 'sunio_woo_product_layout', 'left-sidebar' );
			}
			return $class;
		}
		...
		public static function bs_class( $class ) {
			if ( sunio_is_woo_shop()
				|| sunio_is_woo_tax() ) {
				$class = get_theme_mod( 'sunio_woo_shop_both_sidebars_style', 'scs-style' );
			} elseif ( sunio_is_woo_single() ) {
				$class = get_theme_mod( 'sunio_woo_product_both_sidebars_style', 'scs-style' );
			}
			return $class;
		}
		...
		public static function sidebar_order( $order ) {
			if ( sunio_is_woo_shop()
				|| sunio_is_woo_tax() ) {
				$order = get_theme_mod( 'sunio_woo_shop_sidebar_order', 'content-sidebar' );
			} elseif ( sunio_is_woo_single() ) {
				$order = get_theme_mod( 'sunio_woo_product_sidebar_order', 'content-sidebar' );
			}
			return $order;
		}
		...
		public static function mini_cart_style() {
			...
			wp_enqueue_style( 'sunio-woo-mini-cart', sunio_CSS_DIR_URI .'woo/woo-mini-cart.min.css' );

			// If rtl
			if ( is_RTL() ) {
				wp_enqueue_style( 'sunio-woo-mini-cart-rtl', sunio_CSS_DIR_URI .'woo/woo-mini-cart-rtl.css' );
			}
		...
		public function init() {
			...
			if ( 'on' != get_theme_mod( 'sunio_woocommerce_display_related_items', 'on' ) ) {
			...
			if ( class_exists( 'WooCommerce_Germanized' ) ) {
				...
				add_action( 'sunio_after_archive_product_inner', array( $this, 'woocommerce_germanized' ) );
				add_action( 'sunio_after_single_product_price', 'woocommerce_gzd_template_single_legal_info' );
			}
		...
		public static function infinite_pagination() {
			...
			// Last text
			$last = get_theme_mod( 'sunio_woo_infinite_scroll_last_text' );
			$last = sunio_tm_translation( 'sunio_woo_infinite_scroll_last_text', $last );
			$last = $last ? $last: esc_html__( 'End of content', 'sunio' );

			// Error text
			$error = get_theme_mod( 'sunio_woo_infinite_scroll_error_text' );
			$error = sunio_tm_translation( 'sunio_woo_infinite_scroll_error_text', $error );
			...
			<div class="infinite-scroll-nav clr">
				<div class="alignleft newer-posts"><?php echo get_previous_posts_link('&larr; '. esc_html__( 'Newer Posts', 'sunio' ) ); ?></div>
				<div class="alignright older-posts"><?php echo get_next_posts_link( esc_html__( 'Older Posts', 'sunio' ) .' &rarr;', $wp_query->max_num_pages ); ?></div>
			</div>
		<?php
		}
		...
		public static function body_class( $classes ) {
			...
			if ( 'dropdown' == get_theme_mod( 'sunio_woo_cat_widget_style', 'default' ) ) {
				$classes[] = 'woo-dropdown-cat';
			}

			// Distraction free class
			if ( ( is_cart()
					&& true == get_theme_mod( 'sunio_woo_distraction_free_cart', false ) )
				|| ( is_checkout()
					&& true == get_theme_mod( 'sunio_woo_distraction_free_checkout', false ) ) ) {
				$classes[] = 'distraction-free';
			}

			// My Account page style
			$style = get_theme_mod( 'sunio_woo_account_page_style', 'original' );
			if ( 'side' == $style ) {
		...
		public static function add_custom_scripts() {
			// Register WooCommerce styles
			wp_enqueue_style( 'sunio-woocommerce', sunio_CSS_DIR_URI .'woo/woocommerce.min.css' );
			wp_enqueue_style( 'sunio-woo-star-font', sunio_CSS_DIR_URI .'woo/woo-star-font.min.css' );

			// If rtl
			if ( is_RTL() ) {
				wp_enqueue_style( 'sunio-woocommerce-rtl', sunio_CSS_DIR_URI .'woo/woocommerce-rtl.css' );
			}

			// If products hover style
			if ( 'hover' == get_theme_mod( 'sunio_woo_products_style', 'default' ) ) {
				wp_enqueue_style( 'sunio-woo-hover-style', sunio_CSS_DIR_URI .'woo/hover-style.min.css' );
				wp_enqueue_script( 'sunio-woo-hover-style', sunio_JS_DIR_URI .'third/woo/woo-hover-style.min.js', array( 'jquery' ), sunio_THEME_VERSION, true );
			}

			// If dropdown category widget style
			if ( 'dropdown' == get_theme_mod( 'sunio_woo_cat_widget_style', 'default' ) ) {
				wp_enqueue_script( 'sunio-woo-cat-widget', sunio_JS_DIR_URI .'third/woo/woo-cat-widget.min.js', array( 'jquery' ), sunio_THEME_VERSION, true );
			}

			// If vertical thumbnails style
			if ( 'vertical' == get_theme_mod( 'sunio_woo_product_thumbs_layout', 'horizontal' ) || 'horizontal' == get_theme_mod( 'sunio_woo_product_thumbs_layout', 'horizontal' ) ) {
				wp_enqueue_script( 'sunio-woo-thumbnails', sunio_JS_DIR_URI .'third/woo/woo-thumbnails.min.js', array( 'jquery' ), sunio_THEME_VERSION, true );
			}

			// If quick view
			if ( get_theme_mod( 'sunio_woo_quick_view', true ) ) {
				wp_enqueue_script( 'sunio-woo-quick-view', sunio_JS_DIR_URI .'third/woo/woo-quick-view.min.js', array( 'jquery' ), sunio_THEME_VERSION, true );
				wp_enqueue_style( 'sunio-woo-quick-view', sunio_CSS_DIR_URI .'woo/woo-quick-view.min.css' );
			...
			if ( class_exists( 'TInvWL_Wishlist' ) ) {
				wp_enqueue_style( 'sunio-wishlist', sunio_CSS_DIR_URI .'woo/wishlist.min.css' );
			}

			// If single product ajax add to cart
			if ( true == get_theme_mod( 'sunio_woo_product_ajax_add_to_cart', false )
				&& sunio_is_woo_single() ) {
				wp_enqueue_script( 'sunio-woo-ajax-addtocart', sunio_JS_DIR_URI .'third/woo/woo-ajax-add-to-cart.min.js', array( 'jquery' ), sunio_THEME_VERSION, true );
			}
			...
			if ( 'yes' == get_theme_mod( 'sunio_woo_display_cart_product_added', 'no' ) ) {
				wp_enqueue_script( 'sunio-woo-display-cart', sunio_JS_DIR_URI .'third/woo/woo-display-cart.min.js', array( 'jquery' ), sunio_THEME_VERSION, true );
			}
			...
			if ( true == get_theme_mod( 'sunio_woo_off_canvas_filter', false )
				&& ( sunio_is_woo_shop()
					|| sunio_is_woo_tax() ) ) {
				wp_enqueue_script( 'sunio-woo-off-canvas', sunio_JS_DIR_URI .'third/woo/woo-off-canvas.min.js', array( 'jquery' ), sunio_THEME_VERSION, true );
			}

			// If mobile menu mini cart
			if ( get_theme_mod( 'sunio_woo_add_mobile_mini_cart', true ) ) {
				wp_enqueue_script( 'sunio-woo-mini-cart', sunio_JS_DIR_URI .'third/woo/woo-mini-cart.min.js', array( 'jquery' ), sunio_THEME_VERSION, true );
			}

			// If multi step checkout
			if ( true == get_theme_mod( 'sunio_woo_multi_step_checkout', false )
				&& is_checkout() ) {
				wp_enqueue_style( 'sunio-woo-multistep-checkout', sunio_CSS_DIR_URI .'woo/woo-multistep-checkout.min.css' );
				...
				wp_enqueue_script( 'sunio-woo-multistep-checkout', sunio_JS_DIR_URI .'third/woo/woo-multistep-checkout.min.js', $woo_deps, sunio_THEME_VERSION, true );
			}
		...
		public static function localize_array( $array ) {
			...
			if ( true == get_theme_mod( 'sunio_woo_product_ajax_add_to_cart', false ) ) {
				...
				$array['cart_url'] 			= apply_filters( 'sunio_woocommerce_add_to_cart_redirect', wc_get_cart_url() );
				$array['view_cart'] 		= esc_attr__( 'View cart', 'sunio' );
			}
			...
			if ( true == get_theme_mod( 'sunio_woo_multi_step_checkout', false ) ) {
				...
				$array['no_account_btn'] 		 = esc_html__( 'I don&rsquo;t have an account', 'sunio' );
				$array['next'] 		 			 = esc_html__( 'Next', 'sunio' );
			}
		...
		public static function cart_overlay() { ?>
			<div class="azt-cart-overlay"></div>
		<?php
		}
		...
		public static function get_mini_cart_sidebar() {
			...
			// Style
			$cart_style = get_theme_mod( 'sunio_woo_cart_dropdown_style', 'compact' );

			// Define classes
			$classes = array( 'sunio-cart-sidebar' );
			...
			echo '<div id="sunio-cart-sidebar-wrap">';
				echo '<div class="'. $classes .'">';
					echo '<a href="#" class="sunio-cart-close">×</a>';
					echo '<h4>'. esc_html__( 'Cart', 'sunio' ) .'</h4><div class="divider"></div>';
					echo '<div class="azt-mini-cart">';
						the_widget( 'WC_Widget_Cart', 'title=' );
					echo '</div>';
				echo '</div>';
				echo '<div class="sunio-cart-sidebar-overlay"></div>';
			echo '</div>';

		}

		/**
		 * Adds an opening div "sunio-toolbar" around top elements.
		 *
		 * @since 1.1.1
		 */
		public static function add_shop_loop_div() {
			echo '<div class="sunio-toolbar clr">';
		}
		...
		public static function register_off_canvas_sidebar() {
			...
			register_sidebar( array (
				'name'          => esc_html__( 'Off-Canvas Filters', 'sunio' ),
				'description'   => esc_html__( 'Widgets in this area are used in the off canvas sidebar. To enable the Off Canvas filter, go to the WooCommerce > Archives section of the customizer and enable the Display Filter Button option.', 'sunio' ),
		...
		public static function get_off_canvas_sidebar() {
			...
			if ( function_exists( 'wc_get_template' ) ) {
				wc_get_template( 'azt-off-canvas-sidebar.php' );
			}
		...
		public static function off_canvas_filter_button() {
			...
			// Get filter text
			$text = get_theme_mod( 'sunio_woo_off_canvas_filter_text' );
			$text = sunio_tm_translation( 'sunio_woo_off_canvas_filter_text', $text );
			$text = $text ? $text: esc_html__( 'Filter', 'sunio' );

			$output = '<a href="#" class="sunio-off-canvas-filter"><i class="icon-menu"></i><span class="off-canvas-filter-text">'. esc_html( $text ) .'</span></a>';
		...
		/**
		 * Closes the opening div "sunio-toolbar" around top elements.
		 *
		 * @since 1.1.1
		 */
		...
		public static function loop_shop_per_page() {
			if ( get_theme_mod( 'sunio_woo_shop_result_count', true ) ) {
				$posts_per_page = ( isset( $_GET['products-per-page'] ) ) ? sanitize_text_field( wp_unslash( $_GET['products-per-page'] ) ) : get_theme_mod( 'sunio_woo_shop_posts_per_page', '12' );

			    if ( $posts_per_page == 'all' ) {
			        $posts_per_page = wp_count_posts( 'product' )->publish;
			    }
			} else {
				$posts_per_page = get_theme_mod( 'sunio_woo_shop_posts_per_page' );
		...
		public static function upsell_display() {
			...
			// Get count
			$count = get_theme_mod( 'sunio_woocommerce_upsells_count', '3' );
			$count = $count ? $count : '3';

			// Get columns
			$columns = get_theme_mod( 'sunio_woocommerce_upsells_columns', '3' );
		...
		public static function cross_sell_display() {
			...
			// Get count
			$count = get_theme_mod( 'sunio_woocommerce_cross_sells_count', '2' );
			$count = $count ? $count : '2';

			// Get columns
			$columns = get_theme_mod( 'sunio_woocommerce_cross_sells_columns', '2' );
		...
		public static function add_out_of_stock_badge() {
			if ( function_exists( 'sunio_woo_product_instock' ) && ! sunio_woo_product_instock() ) {
				$label = esc_html__( 'Out of Stock', 'sunio' );  ?>
				<div class="outofstock-badge">
					<?php echo esc_html( apply_filters( 'sunio_woo_outofstock_text', $label ) ); ?>
		...
		public static function loop_product_thumbnail() {
			if ( function_exists( 'wc_get_template' ) ) {
				...
				$style = get_theme_mod( 'sunio_woo_product_entry_style' );
		...
		public static function archive_product_content() {
			if ( function_exists( 'wc_get_template' ) ) {
				if ( 'hover' == get_theme_mod( 'sunio_woo_products_style', 'default' ) ) {
					wc_get_template( 'azt-archive-product-hover.php' );
				} else {
					wc_get_template( 'azt-archive-product.php' );
				}
			}
		}
		...
		public static function quick_view_button() {
			...
			$button  = '<a href="#" id="product_id_' . $product->get_id() . '" class="azt-quick-view" data-product_id="' . $product->get_id() . '"><i class="icon-eye"></i>' . esc_html__( 'Quick View', 'sunio' ) . '</a>';

			echo apply_filters( 'sunio_woo_quick_view_button_html', $button );
		}
		...
		public static function single_product_content() {
			if ( function_exists( 'wc_get_template' ) ) {
				wc_get_template( 'azt-single-product.php' );
			}
		}
		...
		public static function product_next_prev_nav() {
			...
			// Next text
			$next_text = esc_html__( 'Previous Product', 'sunio' );
			$next_text = apply_filters( 'sunio_woo_nav_next_text', $next_text );

			// Prev text
			$prev_text = esc_html__( 'Next Product', 'sunio' );
			$prev_text = apply_filters( 'sunio_woo_nav_prev_text', $prev_text ); ?>

			<div class="azt-product-nav-wrap clr">
				<ul class="azt-product-nav">
			        <?php
			        if ( is_a( $next_post , 'WP_Post' ) ) { ?>
						<li class="next-li">
							<a href="<?php echo get_the_permalink( $next_post->ID ); ?>" class="azt-nav-link next" rel="next"><i class="fa fa-angle-left"></i></a>
							<a href="<?php echo get_the_permalink( $next_post->ID ); ?>" class="azt-nav-text next-text"><?php echo esc_attr( $next_text ); ?></a>
							<div class="azt-nav-thumb">
					...
					if ( is_a( $prev_post , 'WP_Post' ) ) { ?>
						<li class="prev-li">
							<a href="<?php echo get_the_permalink( $prev_post->ID ); ?>" class="azt-nav-text prev-text"><?php echo esc_attr( $prev_text ); ?></a>
							<a href="<?php echo get_the_permalink( $prev_post->ID ); ?>" class="azt-nav-link prev" rel="next"><i class="fa fa-angle-right"></i></a>
							<div class="azt-nav-thumb">
		...
		public static function floating_bar() {
			...
			// Select options text
			$text = esc_html__( 'Select Options', 'sunio' );
			$text = apply_filters( 'sunio_floating_bar_select_text', $text ); ?>

			<div class="azt-floating-bar">
				<div class="container clr">
					<div class="left">
				        <p class="selected"><?php esc_html_e( 'Selected:', 'sunio' ); ?></p>
					...
					<div class="right">
		                ...
		                if ( 'outofstock' == $product->get_stock_status() ) { ?>
		                	<p class="stock out-of-stock"><?php esc_html_e( 'Out of stock', 'sunio' ); ?></p>
		            	<?php
		 ...
		 * @since 1.0.0
		 */
		public static function sunio_before_account_navigation() {
			...
			if ( $current_user->display_name ) {
				...
				$name = esc_html__( 'Welcome!', 'sunio' );
			}
			$name = apply_filters( 'sunio_user_profile_name_text', $name );

			echo '<div class="woocommerce-MyAccount-tabs clr">';
				echo '<div class="sunio-user-profile clr">';
					echo '<div class="image">'. get_avatar( $current_user->user_email, 128 ) .'</div>';
					echo '<div class="user-info">';
						echo '<p class="name">'. esc_attr( $name ) .'</p>';
						echo '<a class="logout" href="'. esc_url( wp_logout_url( get_permalink() ) ) .'">'. esc_html__( 'Logout', 'sunio' ) .'</a>';
					echo '</div>';
				echo '</div>';
		 ...
		 * @since 1.0.0
		 */
		public static function sunio_after_account_navigation() {
			echo '</div>';
		}
		 ...
		 * @since 1.0.0
		 */
		public static function sunio_login_wrap_before() {
			echo '<div class="sunio-loginform-wrap">';
		}
		 ...
		 * @since 1.0.0
		 */
		public static function sunio_login_wrap_after() {
			echo '</div>';
		}
		...
		public static function add_product_classes( $classes ) {
			...
			// Vars
			$product 			= wc_get_product( get_the_ID() );
			$content_alignment 	= get_theme_mod( 'sunio_woo_product_entry_content_alignment', 'center' );
			$content_alignment 	= $content_alignment ? $content_alignment : 'center';
			$thumbs_layout 		= get_theme_mod( 'sunio_woo_product_thumbs_layout', 'horizontal' );
			$thumbs_layout 		= $thumbs_layout ? $thumbs_layout : 'horizontal';
			$tabs_layout 		= get_theme_mod( 'sunio_woo_product_tabs_layout', 'horizontal' );
			$tabs_layout 		= $tabs_layout ? $tabs_layout : 'horizontal';
			$btn_style 			= get_theme_mod( 'sunio_woo_product_addtocart_style', 'normal' );
			...
			if ( $product && ! empty( $woocommerce_loop['columns'] ) ) {
				...
				$classes[] = 'col';
				$classes[] = sunio_grid_class( $woocommerce_loop['columns'] );
				$classes[] = 'azt-content-'. $content_alignment;
			...
			if ( post_type_exists( 'product' ) ) {
				...
				// Thumbnails layout
				$classes[] = 'azt-thumbs-layout-' . $thumbs_layout;

				// Add to cart button style
				$classes[] = 'azt-btn-' . $btn_style;

				// Tabs layout
				$classes[] = 'azt-tabs-layout-' . $tabs_layout;
			...
			// Sale badge style
			$sale_style = get_theme_mod( 'sunio_woo_sale_badge_style', 'square' );
		...
		public static function primary_texts( $texts ) {
			return array_merge( array(
				...
				'.woocommerce .sunio-grid-list a.active',
				'.woocommerce .sunio-grid-list a:hover',
				'.woocommerce .sunio-off-canvas-filter:hover',
				'.widget_shopping_cart ul.cart_list li .azt-grid-wrap .azt-grid a.remove:hover',
		...
		public static function primary_borders( $borders ) {
			return array_merge( array(
				...
				'.woocommerce .sunio-grid-list a:hover',
				'.woocommerce .sunio-off-canvas-filter:hover',
				'.azt-product-nav li a.azt-nav-link:hover',
				...
				'#azt-checkout-timeline.arrow .active .timeline-wrapper:before' => array( 'top', 'bottom' ),
				'#azt-checkout-timeline.arrow .active .timeline-wrapper:after' => array( 'left', 'right' ),
		...
		public static function primary_backgrounds( $backgrounds ) {
			return array_merge( array(
				...
				'.azt-product-nav li a.azt-nav-link:hover',
				'.woocommerce div.product.azt-tabs-layout-vertical .woocommerce-tabs ul.tabs li a:after',
				'.woocommerce .widget_product_categories li.current-cat > a ~ .count',
				'.woocommerce .widget_product_categories li.current-cat > a:before',
				'.woocommerce .widget_layered_nav li.chosen a ~ .count',
				'.woocommerce .widget_layered_nav li.chosen a:before',
				'#azt-checkout-timeline .active .timeline-wrapper',
				'.bag-style:hover .wcmenucart-cart-icon .wcmenucart-count',
				'.show-cart .wcmenucart-cart-icon .wcmenucart-count',
		...
		public static function menu_wishlist_icon( $items, $args ) {
			...
			// Return
			if ( ! class_exists( 'TInvWL_Wishlist' )
				|| true != get_theme_mod( 'sunio_woo_wishlist_icon', false )
		...
		public static function menu_cart_icon( $items, $args ) {
			...
			if ( 'full_screen' == $header_style ) {
				$items .= '<li class="woo-cart-link"><a href="'. esc_url( wc_get_cart_url() ) .'">'. esc_html__( 'Your cart', 'sunio' ) .'</a></li>';
			} else {
		...
		public static function get_cart_icon() {
			...
			<li class="<?php echo esc_attr( $classes ); ?>">
				<?php sunio_wcmenucart_menu_item(); ?>
				<?php
				if ( 'drop_down' == $style
					&& 'full_screen' != $header_style
					&& 'vertical' != $header_style ) { ?>
					<div class="current-shop-items-dropdown azt-mini-cart clr">
		...
		public static function menu_cart_icon_fragments( $fragments ) {
			ob_start();
			sunio_wcmenucart_menu_item();
			$fragments['li.woo-menu-icon a.wcmenucart, .sunio-mobile-menu-icon a.wcmenucart'] = ob_get_clean();
		...
		public static function sale_flash() {
			...
			if ( $product->is_type( 'simple' ) || $product->is_type( 'external' ) ) {
				...
				$percent = sprintf( __( '%s', 'sunio' ), $maximumper );

			} else {

				$percent = '<span class="onsale">' . __( 'Sale!', 'sunio' ) . '</span>';
				return $percent;
		...
		public static function login_register_links() {
			...
			if ( 'original' != get_theme_mod( 'sunio_woo_account_page_style', 'original' ) ) {
				return;
			}
			...
			// Define classes
			$classes = array( 'azt-account-links' );
			...
			// Login text
			$text = esc_html__( 'Login', 'sunio' );

			$html = '<ul class="'. $classes .'">';
				$html .= '<li class="login">';
					if ( 'yes' == $registration ) {
					    $html .= '<a href="#" class="azt-account-link current">'. $text .'</a>';
					} else {
					    $html .= '<span class="azt-account-link current">'. $text .'</span>';
					}
				$html .= '</li>';

				// If registration
				if ( 'yes' == $registration ) {
					$html .= '<li class="or">'. esc_html__( 'Or', 'sunio' ) .'</li>';
					$html .= '<li class="register">';
						$html .= '<a href="#" class="azt-account-link">'. esc_html__( 'Register', 'sunio' ) .'</a>';
					$html .= '</li>';
				}
                ...
                foreach ($fieldset as $key => $field) {
                    ...
                    switch ($fieldset_key) {
                        case 'shipping' :
                            ...
                            $field_label = sprintf(__('Shipping %s', 'sunio'), $field_label);
                            break;
                        case 'billing' :
                            /* translators: %s: field name */
                            $field_label = sprintf(__('Billing %s', 'sunio'), $field_label);
                            break;
                    }
                    ...
                    if (in_array('postcode', $format)) {
                        ...
                        if ('' !== $data[$key] && !WC_Validation::is_postcode($data[$key], $country)) {
                            $errors->add('validation', sprintf(__('%s is not a valid postcode / ZIP.', 'sunio'), '<strong>' . esc_html($field_label) . '</strong>'));
                        }
                    }
                    ...
                    if (in_array('phone', $format)) {
                        ...
                        if ('' !== $data[$key] && !WC_Validation::is_phone($data[$key])) {
                            ...
                            $errors->add('validation', sprintf(__('%s is not a valid phone number.', 'sunio'), '<strong>' . esc_html($field_label) . '</strong>'));
                        }
                    }
                    ...
                    if (in_array('email', $format) && '' !== $data[$key]) {
                        ...
                        if (!is_email($data[$key])) {
                            ...
                            $errors->add('validation', sprintf(__('%s is not a valid email address.', 'sunio'), '<strong>' . esc_html($field_label) . '</strong>'));
                            continue;
                        }
                    }
                    ...
                    if ('' !== $data[$key] && in_array('state', $format)) {
                        ...
                        if (!empty($valid_states) && is_array($valid_states) && sizeof($valid_states) > 0) {
                            ...
                            if (!in_array($data[$key], $valid_state_values)) {
                                ...
                                $errors->add('validation', sprintf(__('%1$s is not valid. Please enter one of the following: %2$s', 'sunio'), '<strong>' . esc_html($field_label) . '</strong>', implode(', ', $valid_states)));
                            }
                        }
                    }
                    ...
                    if ($required && '' === $data[$key]) {
                        /* translators: %s: field name */
                        $errors->add('required-field', apply_filters('woocommerce_checkout_required_field_notice', sprintf(__('%s is a required field.', 'sunio'), '<strong>' . esc_html($field_label) . '</strong>'), $field_label));
                    }
                }
            }
		...
		public function multistep_checkout( $template, $template_name, $template_path ) {
			...
			if ( 'checkout/form-checkout.php' == $template_name ) {
                $template 	= sunio_THEME_DIR . '/woocommerce/checkout/form-multistep-checkout.php';
