 ...
 * @since 1.0.0
 */
function sunio_standard_fonts() {
	return apply_filters( 'sunio_standard_fonts_array', array(
 ...
 * @since 1.0.0
 */
function sunio_enqueue_google_font( $font ) {

	// Return if disabled
	if ( true == get_theme_mod( 'sunio_disable_google_font', false ) ) {
		return;
	}

	// Get list of all Google Fonts
	$google_fonts = sunio_google_fonts_array();
	...
	// Subset
	$get_subsets 	= get_theme_mod( 'sunio_google_font_subsets', array( 'latin' ) );
	$subsets 		= '';
	...
	$weights = apply_filters( 'sunio_google_font_enqueue_weights', $weights, $font );
	$italics = apply_filters( 'sunio_google_font_enqueue_italics', true );
	...
	// Enqueue style
	wp_enqueue_style( 'sunio-google-font-'. $handle, $url, false, false, 'all' );
