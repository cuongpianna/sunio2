wp.customize.controlConstructor['sunio-range'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
			...
			input_default,
			changeAction,
			sunio_range_input_number_timeout;

		// Update the text value
		jQuery( 'input[type=range]' ).on( 'mousedown', function() {

			range 			= jQuery( this );
			range_input 	= range.parent().children( '.sunio-range-input' );
		...
		function sunio_autocorrect_range_input_number( input_number, timeout ) {
			...
			clearTimeout( sunio_range_input_number_timeout );

			sunio_range_input_number_timeout = setTimeout( function() {
		...
		jQuery( 'input.sunio-range-input' ).on( 'change keyup', function() {

			sunio_autocorrect_range_input_number( jQuery( this ), 1000);

		} ).on( 'focusout', function() {

			sunio_autocorrect_range_input_number( jQuery( this ), 0);

		} );

		// Handle the reset button
		jQuery( '.sunio-reset-slider' ).on('click', function() {
