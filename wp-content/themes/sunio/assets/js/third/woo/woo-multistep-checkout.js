...
$j( document ).on( 'ready', function() {
    ...
    sunioWooMultiStepCheckout();
} );

/* ==============================================
WOOCOMMERCE MULTI-STEP CHECKOUT
============================================== */
function sunioWooMultiStepCheckout() {
	"use strict"
    ...
    form_actions.find( '.button.prev' ).add( '.button.next' ).on( 'click', function( e ) {
        var $this 			= $j( this ),
            timeline 		= $j( '#azt-checkout-timeline' ),
            ...
            is_logged_in    = sunioLocalize.is_logged_in,
            button_title,
            valid           = false,
        ...
        if ( type == 'billing' || type == 'shipping' ) {
            ...
            var data = {
                action: 'sunio_validate_checkout',
                type: type,
        ...
        if ( valid ) {
            ...
            if ( action == 'next' ) {
                ...
                $j( 'html, body' ).animate( {
                    scrollTop: $j( '#azt-checkout-timeline' ).offset().top - $offset },
                'slow' );
            } else if ( action == 'prev' ) {
            ...
            if ( ( current_step == 1
                ...
                ( is_logged_in == false
                	&& ( ( current_step == 0
                		&& sunioLocalize.login_reminder_enabled == 1 )
                	||  ( current_step == 1
                		&& sunioLocalize.login_reminder_enabled == 0 ) ) ) ) {
                prev.fadeOut( 120 );
            } else {
                prev.fadeIn( 120 );
            }
            ...
            if ( is_logged_in == false
                && ( ( current_step == 0
                    ...
                    ||  ( current_step == 1
                        && sunioLocalize.login_reminder_enabled == 0 ) )  ) {
                next.val( sunioLocalize.no_account_btn );
            } else {
                next.val( sunioLocalize.next );
            }
